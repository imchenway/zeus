...
## ZEUS-0267 任务推送状态联动撤销契约

- “推送到新会话”只负责创建会话和首轮输入，不读取任务当前管理状态决定额外确认步骤，也不自动把任务迁移到项目推送后状态。
- Renderer 不维护任务状态乐观投影、后台重试或“任务状态未同步”会话警告；会话创建失败只表达会话创建本身的真实结果。
- Local Server 不提供任务推送专用状态同步接口；任务管理状态继续通过统一的状态修改接口由用户显式操作。
- 任务列表、任务详情、会话标题和侧边栏分组继续读取同一真实任务状态；创建会话不会制造任务事件、状态审计或分组跳动。
- 新旧执行宿主并存时，新界面不得调用旧宿主可能不存在的任务推送附属接口；会话主路径只依赖创建会话所需能力。

优点：任务推送没有额外状态确认和兼容性依赖，旧宿主不会因缺少附属路由在会话中持续报错，任务状态也不会被会话动作隐式改写。

代价：任务开始开发后需要用户在任务状态入口手动调整阶段，侧边栏分组不会因推送自动移动。

原设计记录保留在 `docs/ZEUS-0267_任务推送即时进入开发中.md`；撤销原因、实现和验证见 `docs/TASK_20260814_001_移除任务推送状态同步校验.md`。
...
- 队列管理区继续提供编辑、删除、排序、引导、恢复和真实状态说明，但不得在默认态重复展示或替代时间线正文；展开后只允许显示单行管理摘要和附件数量，编辑态才显示完整输入。尚未确认进入会话记录的恢复输入必须先合并并持久化到输入框，再撤下时间线投影和服务端确认记录，不得因实时队列与完整快照入口不同而丢失。
