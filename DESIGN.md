...
- 用户点击发送后，原消息立即以稳定 `clientUserMessageId` 留在时间线，并持续显示“发送中”“引导中”或“排队中”；Provider 轮次开始后由既有“正在思考”和真实过程内容接管反馈。
...
优点：用户从点击发送开始始终能区分 Zeus 正在提交、Provider 正在处理、排队、结果不确定和明确失败，单条消息异常不会再伪装成整条会话卡死。

代价：失败消息会与已恢复到输入区的同一内容同时可见；这是为了保留失败审计和可编辑恢复，Renderer 必须继续使用客户端消息身份避免后续重试产生错误替换。

完整诊断、实施和验证记录见 `docs/ZEUS-0220_会话消息发送状态可见性.md`。
...
## ZEUS-0267 任务推送即时状态契约

- 任务处于项目默认状态时，用户确认推送后，任务列表、任务详情、会话标题和侧边栏会话分组必须在同一界面更新中进入项目配置的推送后状态；新会话行首次出现时不得先落入默认状态分组。
- 任务已经处于推送后状态时直接推送。处于其他任何状态时，现有推送弹窗原位提供“改为推送后状态并推送”和“保留当前状态并推送”，终态只有用户明确选择时才重开。
- 用户确认进入推送后状态后，会话创建失败、首轮失败或等待重试都不回退任务状态；任务管理状态只表达用户确认的工作阶段，不再证明已有成功运行的会话。
- 状态持久化与会话创建并行执行。状态同步失败不得阻断会话，也不得让当前界面跳回旧分组；会话工作面必须原位显示“任务状态未同步”和重试动作，后台可继续重试。
- 状态同步必须以推送确认时的原状态为保护条件。用户随后手动选择其他状态时，人工事实优先，停止本次推送的后台状态重试，不允许迟到结果再改回推送后状态。
- 状态同步成功必须记录任务事件、审计记录和实时任务更新；“保留当前状态并推送”不得产生伪造的状态变化记录。

优点：用户确认后立即得到稳定一致的工作阶段和侧边栏位置，会话链路失败也不会造成任务状态来回跳动。

代价：推送后状态不再等价于首轮已经启动；Renderer 需要维护短暂的乐观投影、同步失败提示、重试和人工覆盖边界。

完整决策、实现和验证记录见 `docs/ZEUS-0267_任务推送即时进入开发中.md`。
