...
### ZEUS-0324 已有任务分支续推契约

- Worktree 推送必须明确区分“创建新的任务分支”和“继续已有任务分支”；继续操作仍创建新会话，只复用原任务环境、worktree、cwd 和分支，不复用旧 Provider 线程。
- 单仓任务以真实任务分支作为选择身份；多仓任务以同一次推送形成的完整任务环境作为选择身份，必须整体复用全部仓库分支，不允许把部分旧分支与部分新分支拼成无法恢复的执行根。
- 能力接口只投影当前任务的真实环境和仓库分支；内部环境 ID 只用于请求身份，不作为用户文案。已回收环境不展示，已合入或已放弃成员及存在活动可写会话的环境不可选择。
- 弹窗能力只用于展示和预选；提交时服务端必须再次复验项目、任务归属、环境状态、成员状态和活动写入，状态变化时保留表单并明确拒绝，不能退回静默新建分支。

优点：同一开发线可以开启多个连续的新会话，模型始终回到原执行目录，多仓可写根不会缺失。

代价：多仓环境只能整体继续；上一条可写会话结束前不能在同一环境开启第二条可写会话。
