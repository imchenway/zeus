...
- Main 完成资源归属、URL 和打开目标复验后，必须先创建并发布加载中的内置浏览器标签，让会话浏览器分栏立即进入可交互状态；页面导航继续在标签内部异步完成，不得等待页面加载完成后才展示浏览器。
- 页面加载失败必须保留已打开标签并通过浏览器错误状态反馈，不静默关闭标签或改用系统浏览器。Agent 在标签打开后操作页面内容时必须显式等待加载状态或目标元素，不能把标签打开冒充为页面已经就绪。
- 自定义文件处理器使用结构化 argv，不通过 shell 字符串拼接路径、行号或用户输入。

优点：用户点击后立即看到真实浏览器与加载状态，慢网页不会被误解为 Zeus 没有响应；安全复验、会话归属和原生页面生命周期仍保持不变。

代价：资源打开回执只证明标签已经创建，不再证明页面内容已加载；自动化和后续页面操作必须使用明确等待条件。
...
- 列表级状态必须读取服务端队列快照中的真实等待原因和后续动作：当前回复、选择或审批、计划确认、执行现场准备、会话中断、连接和恢复；不得自行猜测成其他会话占用名额，也不得用孤立的“排队中”代替解释。
- 正常等待条件解除后沿用协调器顺序自动派发，不额外要求用户确认。中断、归档恢复和写入不确定等安全场景继续保持暂停或回到草稿确认。
- Zeus 不设置项目级或全局会话并发上限；不同会话直接交给各自供应商处理并发、排队与限流。单个会话仍然只允许一个活动执行轮次，后续消息继续先进先出。
- 协调器无法派发时必须把错误和恢复要求持久化到 submission，并发布包含同一事实的队列快照；禁止只广播一次错误后让消息保持无法解释的 `queued`。
...
代价：Zeus 不再主动为供应商削减跨会话峰值；供应商限流、排队或拒绝必须由各供应商链路准确返回并展示。
...
- 该契约只收紧 Codex 引导语义，不改变普通排队和自动派发、Pi 消息协议、单会话串行及用户问答协议。
...
## ZEUS-0304 非阻断更新窗口焦点契约

- 用户显式点击检查更新、侧边栏更新提醒或更新通知时，原生更新窗口可以获得一次键盘焦点，作为该次显式操作的直接反馈。
- 检查、下载、校验、重试等后续状态只更新已有窗口内容，不得调用应用激活、成为键盘主窗口、从最小化中恢复或再次置前。
- 用户切回 Zeus 主窗口或其他应用后，原有键盘焦点必须保持；Homebrew 进度更新频率不得影响电脑的其他输入操作。
- 自动检查和自动预取继续保持后台执行，不因状态刷新自动创建、显示或激活更新窗口。
- 精确重启失败发生在旧 Zeus 已退出之后，不存在可继续操作的主窗口；此时失败窗口仍可以显式激活，确保用户能看到恢复信息。

优点：显式打开仍有符合 macOS 预期的立即反馈，后台进度不会再打断主窗口或其他应用输入。

代价：原生协议需要严格区分“显式显示”与“被动刷新”；辅助程序异常重启后，被动恢复的窗口可能留在当前活动窗口后方，需要用户再次点击更新入口才会聚焦。
...
- 计划控制动作必须标识为“计划操作”，不得提供引导、编辑、删除或调整顺序的能力；普通消息不得越过它，服务端同时拒绝绕过界面的改写与重排。当前会话空闲后直接交给供应商，不再等待 Zeus 全局容量。
...
代价：计划确认未处理期间，普通后续消息会等待用户明确决策；确认后的动作不再由 Zeus 跨会话削峰，供应商限流需要作为真实派发结果处理。

完整现场、根因、取舍与验证记录见 `docs/ZEUS-0297_计划模式确认消息错误.md`。

## ZEUS-0302 命令执行历史视觉契约

- 命令执行历史继续使用居中弹窗，保留历史列表、执行详情、实时日志、停止操作与产物预览；桌面宽高上限在 ZEUS-0252 基础上适度增加，不转为近全屏工作台。
- 弹窗宽度上限使用 `1320px`，主内容高度上限使用 `720px`；实际尺寸继续受视口安全边距限制，窄窗口继续使用单栏和局部滚动。
- 终端日志复制入口使用 `28px` 纯图标工具条按钮，不使用白色胶囊或带文字的成功块；默认、复制中、已复制、日志过大和失败使用不同图标并保留非颜色语义。
- 已复制勾选约两秒后恢复复制图标，便于对运行中日志重复取得新快照；可访问名称、悬停提示和 `aria-live` 必须继续显示完整文字状态。
- 本契约只收敛尺寸和视觉反馈，不改变“本次运行完整日志”、32 MiB 上限、Electron 剪贴板往返校验、实时轮询或日志跟随语义。

优点：大屏下路径、日志与产物的阅读空间更多，复制操作回到低噪音工具条层级。

代价：弹窗仍保留局部滚动，纯图标按钮的完整文字含义需要通过悬停或读屏获取。
...
## ZEUS-0305 会话归档状态漂移恢复契约

- 恢复 Codex 会话时，Zeus 本地 `providerState=archived` 只是上次持久投影，不能替代 Provider 当前线程事实。
- `thread/unarchive` 明确返回“找不到已归档 rollout”时，只允许对同一 Provider thread ID 继续执行原生续接与权威快照读取；两步都成功后才把 Zeus 会话恢复为未归档。
- 恢复期间补齐的历史文件变更若包含无效或越出会话执行根的路径，继续拒绝该路径且不生成可审核、撤销或重新应用的文件变更；单条已拒绝的历史资源不能阻断同一线程其余消息和状态恢复。实时文件变更及其他历史补齐错误仍按原规则失败关闭。
- Provider 线程真实缺失、身份不一致、续接失败或权威快照不可读时继续失败关闭，保留 Zeus 归档记录和恢复入口；不得吞掉其他 `thread/unarchive` 错误。
- 状态漂移收敛不创建新会话、不复制或移动 JSONL、不重发用户消息，也不改变任务管理状态、工作区或 Git 事实；任务重开与设置页恢复复用同一编排。

优点：Provider 已经可用时，用户点击恢复即可回到原会话，不需要手改数据库或搬运文件；历史坏资源也不会永久锁死整条会话。代价：恢复链路需要窄错误分类和同一线程的二次权威复验，被拒绝的越界文件记录不提供文件卡或恢复操作。

本项是可回退的恢复编排，不新增 ADR。现场证据、实现与验证记录见 `docs/ZEUS-0305_归档会话恢复报错.md`。
...
## ZEUS-0313 底部交互坞固定位置契约

- 普通用户询问、计划实施确认、命令、文件、权限和 MCP 请求统一占用会话底部交互坞；交互坞位于会话时间线之外，消息滚动不得带动它移动。
- 交互坞与普通会话输入区互斥。存在阻塞交互请求时暂时替换输入区；回答、跳过或关闭请求后恢复输入区与既有焦点回交。
- 会话时间线承担剩余可用高度并独立滚动；交互坞不得使用覆盖消息的固定或悬浮定位。长询问压缩时间线可视高度，超限内容只在交互坞内部滚动。
- 待回答询问不属于会话记录；请求解决后生成的已回答询问记录继续按答案提交时间进入会话时间线，并随历史消息滚动。
- 本契约只覆盖 ZEUS-0306 将普通用户询问移入会话记录滚动区的布局口径；该任务的队列展示、Provider 接收后迁移、稳定消息身份和滚动跟随规则保持不变。

优点：待处理主操作始终位于稳定位置，不会因阅读历史或新消息增长离开视口，也不会覆盖会话内容。

代价：长询问会减少消息时间线的即时可视高度，需要在底部交互坞内独立滚动查看全部选项。

完整实施与验证记录见 `docs/ZEUS-0313_选项卡位置固定.md`。

## ZEUS-0311 统一会话、队列、用量与处理过程界面契约

- 会话页的主对象始终是产品会话。模型下拉框改变下一次提交的输入预设，不得让标题、导航身份或已显示时间线提前切换到另一个 Provider 分段。
- 未获运行时接纳的 submission 只显示在独立队列区域，不进入正式 transcript；队列必须展示排队、暂停、结果未知、配置变化和 replacement 关系，不能用普通用户消息气泡伪装已发送。
- App Server 或 Pi 在产生原生 turn 前返回确定错误时，统一记为“运行时明确拒绝”：候选分段废弃、切换锁释放、来源分段保持 current，允许人工 replacement；只有请求实际写出后失去回执才进入结果未知恢复保护。
- Pi 进入 Agent run 前必须同步触发预检回调；`prompt` 返回时仍未回调即中止本轮并按写入前失败收口，禁止让 submission、租约和队列永久停在派发中。Pi 已接纳活动轮次随 Zeus 进程中断后，重启恢复必须标记为中断、保留 current 分段和已确认历史、暂停后续队列且禁止自动重发。
- 重启恢复必须先把 turn、submission、队列和会话状态收敛，再写入带 `segment_id/event_kind/payload_json` 的恢复事件与持久警告；恢复证据表的建表列必须与唯一恢复仓储完全一致，任何证据写入失败都不得被当成恢复成功。
- Codex `additionalContext` 只允许 app-server v2 命名条目 `{ kind, value }`；`zeus_manifest` 为可信应用清单，`zeus_history` 与旧会话显式引用为不可信字符串正文。当前用户消息只存在于 `turn/start.input`，冲突准备等内部信封只保存在 `operationContext`。
- 模型连接中用户或隔离验收明确保存的 `contextWindow`、`maxTokens` 是该 route 的执行事实；自动能力目录只能补齐缺失能力，不能在加载时静默覆盖持久值。Pi 压缩保留量必须按该实际窗口缩放，Codex 临时压缩只依赖订阅到的 turn/item 事件，不假设临时 thread 支持历史列表接口。
- 失败或结果未知的队首必须突出它阻塞了后续队列，并只提供重试、改路由 replacement、取消和查看详情。上移、下移或立即发送不能绕过该队首。
- 成功模型切换保持安静，不插入普通“已切换分段”标记。能力损失、配置回显不一致、Provider 晚到活动、历史缺口和迁移恢复才使用持久警告。
- 活动 turn 直接展开当前步骤；完成 turn 折叠为“处理过程”。展开后按稳定顺序显示全部思考摘要、工具、命令、重试、上下文压缩、等待和警告，不按同轮类型只保留最后一项。
- 可读 reasoning 的固定术语是“思考摘要”，过程集合固定为“处理过程”。界面、辅助文本和无障碍名称都不得使用“隐藏思维链”“完整思考过程”等无法证明的说法。
- 上下文圆环只使用最后一次真实模型请求的 input Token 和该请求实际模型的窗口。会话累计值、turn 累计值、预检估算和自动压缩阈值必须分开显示。
- 只要统一用量快照已经存在，即使 `latestModelRequest` 为空，上下文圆环也必须显示未知，不能回退旧 Provider 账本；旧账本只服务于完全没有统一用量结构的兼容会话。
- Token 单位统一为 `128K Token`、`1M Token` 等模型计量；KB/MB 只用于原始文件大小，不得用于模型上下文。
- 推理强度证据按“用户选择、Zeus 冻结、适配器已传递、运行时已接纳、Provider 已回显”分层展示。若 Provider 未回显 `max`，必须明确显示“实际内部档位不可验证”，不得用 reasoning Token 数量补证。
- 结构代次不匹配时停止渲染会话业务内容，显示本地服务与 Renderer 不兼容的明确错误；不得猜测旧字段或混合应用新旧快照。
- 水合快照带 `throughEventSeq`。Renderer 在水合期间缓存实时事件，快照应用后只消费更大序号，避免旧事件回插或重复过程项。
- 迁移未完成时只显示维护工作面。该工作面说明当前阶段、失败影响和下一步，提供重试、打开诊断位置和退出；正常本地 HTTP 服务、会话页和 Provider 派发不得提前启动。
- 迁移失败必须区分原子提升前与提升后：提升前失败可以说明正式库未替换；提升后校验失败必须明确正式路径已换成候选库，并通过诊断和重试收敛，禁止展示相反事实。

优点：用户能区分“本机已保存”“运行时已接纳”“Provider 配置已回显”三层事实，模型切换不会破坏会话身份或顺序。代价：队列和配置证据需要更多状态表达，阻塞队首会让后续输入等待用户处理。

本节覆盖 ZEUS-000041 与 ZEUS-0288 中把未接纳队列正文放入正式 transcript 的旧展示约束；稳定身份、失败保留、键盘操作和恢复能力继续保留在独立队列区域。
