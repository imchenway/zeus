...
## ZEUS-0334 页面即时切换与后台收敛契约

- 页面或会话身份变化属于紧急交互：选中态、目标标题、目标正文与输入区不得等待 HTTP、Provider、Git、模型能力或后台校准完成后才提交。
- 当前项目近期原生会话允许以有界内存后台预读权威快照；预读不得建立额外 Provider 线程、执行外部 AI CLI、常驻 WebSocket 或挂载全部会话控制器。
- 命中热缓存时首帧直接展示消息、询问选项和正文，随后由目标会话控制器建立真实事件流并原地校准；校准失败保留已有真实内容并使用统一错误出口，不闪回空白或旧会话。
- 项目级模型能力使用缓存先显示、后台刷新；缓存只用于界面连续性，服务端继续复验模型、权限、服务档位和执行能力。
- 侧边栏选中变化不得触发所有会话行的布局测量；新增、移除和真实重排仍保留列表动效与 reduced-motion。
- 已完成轮次中默认收起的工具、命令和过程正文只在用户展开时构建；最终回答、计划、变更、失败、时长和已回答询问继续首帧展示。
- “推送到新会话”在能力读取前先显示项目上次成功记住的模型与 Worktree 等选择，能力返回后再归一化；禁止先显示硬编码默认值再跳到记忆值。

优点：用户点击后的可见身份和已有内容接近瞬时出现，网络与复杂历史渲染不再阻塞页面首帧。

代价：近期会话热缓存会增加有上限的内存占用，极早点击、缓存淘汰或首次展开超长历史过程仍可能产生局部加载成本；缓存能力在后台刷新前可能短暂落后，但不能用于绕过服务端事实。

## ZEUS-0318 登录与配置导入分步弹窗契约

- “推送到新会话”弹窗只负责本次模型、工作区、权限、上下文与首发内容确认；账号登录和 Codex 配置导入不得作为它的正文卡片或共享其长滚动区。
- 用户点击“登录并继续”后，切换到独立的“登录 Zeus 专属 Codex”弹窗。首次检测到安全可导入配置时，再切换到独立配置导入弹窗；导入、暂不导入或启用完成后回到同一登录步骤，不要求重新填写推送表单。
- 登录弹窗负责准备、等待官方网页、登录成功和返回 Zeus 四类状态；配置导入弹窗只负责预览、跳过、导入、启用和启用失败。任一时刻只能有一个模态主操作面，禁止把两个步骤同时展示。
- 用户取消登录或在配置预览中返回时，恢复原推送弹窗并保留模型、工作区、权限、补充信息与本次附件；已经写入但尚未启用的配置继续保持真实“等待启用”门禁，不得绕过后直接创建会话。
- 官方网页只承担账号授权。Zeus 以 app-server 权威账号状态判定成功，登录成功后主动激活发起窗口；为覆盖外部完成页的尾随导航，在成功反馈前后各激活一次，再继续原推送。
- Zeus 不接管 OAuth 凭据、不伪造第三方回调，也不把 ChatGPT 或 Codex App 当作本次流程的完成目的地。当前协议没有 Zeus 品牌值，因此关闭托管产品成功页，由 Zeus 自己的登录弹窗承担产品收口。

优点：三个步骤各自只有一个目标，长推送表单不再混入账号和文件迁移决策；取消与失败都能回到原配置；网页授权完成后用户回到实际发起操作的 Zeus。

代价：登录过程会在三个独立弹窗之间切换；系统浏览器标签仍由用户或浏览器管理，Zeus 不能替第三方网页关闭标签；完整验收必须使用隔离 `Zeus Test.app` 完成一次真实网页登录往返。
