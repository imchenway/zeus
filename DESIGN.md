...
- 原生会话底部 composer 在连接、等待结构化交互、上一轮失败、底层线程不可续等状态下均保持可见和可编辑；结构化交互坞位于 composer 上方，不再替换 composer。
- 用户发送消息即触发自动续接：能安全继续原线程时进入原线程；不能继续时自动创建承接会话，并继承原任务及可验证的执行现场。
- 普通界面不显示“需要恢复”或要求用户先点击“新建会话”；承接失败时保留输入并说明无法继续的真实原因。
- 优点：任何状态都有明确主输入，用户无需理解运行内核恢复状态；代价：等待结构化问题时会同时出现问题卡和输入框，界面需要保持清晰层级。
- 本修订覆盖本节中“交互坞与 composer 不同时出现”的旧规则；结构化问题仍必须通过对应问题卡回答，普通输入按会话消息排队。
...
- 勾选路径在提交动作发生时通过受控 `git add -A -- <paths>` 写入 index；提交前拒绝未解决冲突和正在被活动会话写入的 worktree。
...
- 用户可以采用来源、采用任务、两者都采用或手工编辑结果。Codex 协助必须由用户显式触发，只把建议写入中央编辑区；用户保存并暂存后才能完成合入。
