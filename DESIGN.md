...
- 任务推送到模型前必须选择“创建新的任务分支”或“使用该任务已有分支”。新分支统一使用 `zeus/` 前缀，并通过 `git check-ref-format --branch` 校验。
- 来源分支必须是主仓库中的本地命名分支；任务工作区必须绑定一个真实远端，优先使用 `origin`，否则使用仓库首个远端。
...
- 任务推送到模型前必须为本次推送逐仓选择来源分支并确认新的任务分支；不得提供或默认使用该任务已有分支。新分支统一使用 `zeus/` 前缀，并通过 `git check-ref-format --branch` 校验。
- 来源分支必须是主仓库中的本地命名分支；任务工作区必须绑定一个真实远端，优先使用 `origin`，否则使用仓库首个远端。
