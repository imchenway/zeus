...
## ZEUS-0293 任务看板开放画布契约

- 任务页“列表 / 看板”是项目级一级视图；看板继续使用真实任务字段、项目状态配置和持久排序，不创建展示专用的伪状态或 mock 任务。
- 看板主视觉是开放冷白画布上的等高细列。状态色只承担定位语义，限于列头圆点、卡片左侧细线和拖动落点；不得重新铺成整列彩色背景或粗边框。
- 任务卡使用中性表面、紧凑字段和轻阴影。标题是第一信息，编号和属性退后一级；移动、隐藏和拖动等低频动作保留完整键盘与读屏能力，但不常驻占满卡片宽度。
- 看板设置属于视图动作，必须与“列表 / 看板”切换相邻；看板正文不得再增加一条只显示分组说明和设置按钮的独立命令条。
- 列、卡片和设置可以按需懒加载；动态分组、子分组、筛选、排序、统计、颜色规则、三种详情方式与并发冲突契约保持不变。

优点：任务本身重新成为首屏视觉主体，状态可扫读且长期使用更低疲劳；设置入口与视图切换的产品语义一致。

代价：弱化大面积状态色后，用户需要依靠列标题和圆点辨识阶段；低频动作需要悬停或键盘聚焦后才更明显。

实施与验证记录见 `docs/ZEUS-0293_任务页看板模式.md`。

## 会话助手交付资源契约

- 图片、文件或网页只有具备明确助手交付身份时，才从工具活动提升为会话正文资源；普通工具截图、页面预览、命令输出和临时富内容块继续属于处理过程。
- 编排层优先通过 `deliverables` 或资源 `delivery=assistant` 声明交付。兼容 `generatedImage` 时，必须同时存在图片内容块、稳定生成回执和当前 Provider 线程受控目录中的真实文件。
- 服务端必须复验真实路径、当前线程目录、普通文件和媒体文件头；不得信任 Renderer 传入绝对路径，也不得把 base64 图片或音频持久化到数据库或发送给 Renderer。
- 助手交付资源不进入活动分组和轮次过程折叠。纯图片使用生成图预览；文件、网页或混合结果使用资源卡。音视频没有专用播放器时可以退化为受信文件卡，但不能再次消失。
- 同一工具条目中没有交付身份的其他资源不得跟随提升。旧的 Provider 顶层生成图历史可以走兼容回退，不能据此放宽新资源的交付判定。

优点：最终产物刷新后仍可见，同时避免把调试截图和临时预览污染到会话正文。

代价：编排层需要显式声明普通文件、网页和音视频交付；无法从缺少标记的旧富内容安全猜测用户是否需要它。

诊断、实现和验证边界见 `docs/TASK_20260815_002_Zeus工具生成图片未进入最终回答诊断.md`。
