...
## ZEUS-0306 会话排队消息与阻塞询问布局契约

- 会话空闲时明确发送的消息立即进入会话记录，并自动定位到最新内容；后续模型输出在用户没有主动上滚时继续自动跟随底部。
- 活动轮次期间通过 Enter 或发送按钮形成的下一轮队列消息，在 Provider 真正接收前不得进入会话记录；它只显示在会话输入区正上方的后续消息区。
- 后续消息区存在队列消息时默认展开，完整显示全部待处理消息，并保留编辑、删除、排序、引导、暂停恢复和真实等待原因。
- 队列消息取得真实 Provider turn 或用户消息事实后才进入会话记录；进入时复用稳定客户端消息身份，并自动定位到最新内容，不得同时保留输入区队列副本。
- 阻塞式普通用户询问继续替换普通会话输入区，但询问卡进入当前会话内容末尾，与最后一条过程反馈保持正常内容间距；不得用新轮次回复占位把询问卡压到窗口底部。
- 新轮次发送不再创建占据大部分对话区的空白回复占位；用户主动上滚后才停止自动跟随，并提供“返回最新消息”。
- 回答批注发送后按条展示回答引用和用户评论，数量只作为辅助标题；本地结构化锚点与 Provider 可读序列化继续保留，但用户消息不得再用 `Response annotations (N)` 或单一数量胶囊替代真实内容。
- 回答批注编辑浮层锚定选区末端或可见编号锚点，窗口滚动和缩放时重新计算；浮层在视口空间不足时上下翻转并约束横向位置，不固定到窗口右下角，也不持续追随鼠标。
- 回答批注编辑浮层使用紧凑桌面密度：引用只在编辑阶段限行预览，评论输入从两行起步并允许有限纵向扩展；发送后的消息引用与评论不截断。
- 本节覆盖 ZEUS-000041 中“队列消息只在对话记录底部显示”和 ZEUS-0288 中“排队消息立即进入时间线”的旧视觉口径；持久队列、稳定消息身份、Provider 去重和失败恢复规则保持不变。

优点：会话记录只展示模型已经接收的真实对话，队列管理始终靠近输入动作，阻塞询问与触发它的过程上下文连续可读，也不再出现无意义的大块空白。

代价：队列消息从后续消息区进入会话记录时会发生一次明确的区域迁移；默认展开多个长消息会占用更多输入区上方空间，但用户仍可手动收起。

本项是可回退的消息投影、滚动和布局规则，不新增架构决策记录。实施与验证记录见 `docs/ZEUS-0306_会话排队消息与阻塞询问布局修复.md`。
