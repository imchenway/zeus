...
- 新建项目会话时可选择“项目目录”或“新建工作树”；项目目录支持搜索、切换本地分支，以及创建并检出新分支，已被其他工作树占用的分支不可切换。工作树可选择本地或远程来源分支并设置新分支名，未提交修改保留在项目目录。会话列表以目录和分支图标区分工作位置，悬停可查看路径。
...
- 在“数字团队”中按工作依赖组织员工协作，可选负责人分工、成果核对和人工确认；普通只读协作无需 Git，研发预设按需加入隔离开发与代码集成。运行冻结流程和成员，负责人仅能向已授权成员追加分工。任务中的新工作安排统一从此入口创建，旧配方和未启动安排导入为模板，已有运行保留原记录。
...
Codex 程序在“设置 → 模型供应商”中单独管理。后台和“检测更新”只检查版本；Zeus 管理的安装需点击明确版本的安装按钮才会更新，自行安装的 Codex 请使用原安装方式更新。安装后若连接失败，先点击“重新连接 Codex”；Zeus 不会自动降级或用旧数据覆盖对话。检测到最新版不代表已经完成所有会话功能的兼容验收。
