...
export function MarkdownStreamingQaApp() {
    ...
    const startScenario = useCallback((nextScenario: MarkdownQaScenario) => {
        ...
        animationFrame = requestAnimationFrame(() => {
            ...
            if (rootRef.current) mutationObserver.observe(rootRef.current, {
                childList: true,
                characterData: true,
                subtree: true
            });
    ...
    return (
        <main className="macos-ai-app zeus-shell session-codex-parity-v1 qa-page qa-markdown-stream-page"
              data-theme="light" data-testid="markdown-stream-fixture">
            ...
            <section className="qa-markdown-stream-controls">
                ...
                <label>
                    输入响应探针
                    <input value={probeValue} onChange={(event) => setProbeValue(event.currentTarget.value)}
                           placeholder="运行期间仍可输入"/>
                </label>
          ...
          {scenario === 'idle' ? '就绪' : scenario === '20kb' ? `${text.length} / ${markdownFixture20kb.length}` : `${text.length} / ${markdownFixture100kb.length}`}
            ...
            <section className="qa-markdown-stream-grid">
                ...
                <aside className="qa-markdown-stream-results" data-testid="markdown-stream-results"
                       data-pass={result?.passed ?? undefined}>
                    <h2>现场指标</h2>
                    {result ? (
                        <dl>
                            <dt>场景</dt>
