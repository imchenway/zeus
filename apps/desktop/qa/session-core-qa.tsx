...
import {
    ...
    sessionReducer
} from '../src/renderer/session/sessionReducer.js';
...
import {
    ...
    reconcileConversationHistoryCache
} from '../src/renderer/session/conversationSnapshotV2Adapter.js';
...
const interruptedQueueTakeoverDurableItem: NativeSessionItemBuffer = {
    ...motionItem('phase-queue-takeover', 'userMessage', 'completed', interruptedQueueTakeoverText, {
        role: 'user',
...
function lateFailureNoRefillState(): NativeSessionState {
    ...
    const withLaterDraft = sessionReducer(sessionReducer(started, {
        type: 'draft_changed',
        draft: '用户后来输入的新草稿'
    }), {
...
function historyPagingRangeSnapshot(input: {
    through: number;
    oldest: number;
    cursor: string;
    hasMore: boolean
...
const historyPagingRangeEvidence = {
    sameHighWater: reconcileConversationHistoryCache(historyPagingCachedRange, historyPagingRangeSnapshot({
        through: 96,
        oldest: 49,
        cursor: 'fresh-tail',
        hasMore: true
    })).snapshot.v2Paging?.history.nextCursor,
    overlappingTail: reconcileConversationHistoryCache(historyPagingCachedRange, historyPagingRangeSnapshot({
        through: 120,
        oldest: 73,
        cursor: 'fresh-overlap',
        hasMore: true
    })).snapshot.v2Paging?.history.nextCursor,
    disconnectedTail: reconcileConversationHistoryCache(historyPagingCachedRange, historyPagingRangeSnapshot({
        through: 200,
        oldest: 153,
        cursor: 'fresh-gap',
        hasMore: true
...
function subagentItem(id: string, type: string, status: string, text: string, updatedAt: string, phase = 'prework'): NativeSubagentThreadSnapshot['turns'][number]['items'][number] {
    return {
        ...
        phase,
        text,
        payload: type === 'reasoning' ? {summary: [text]} : type === 'fileChange' ? {
            changes: [{
    ...
    return (
        <main className="macos-ai-app zeus-shell session-codex-parity-v1 qa-page qa-defect-page theme-light">
            ...
            <section className="qa-implementation-panel qa-defect-transcript" data-testid="zeus-0323-transcript">
                <ConversationTranscript state={defectSessionState} language="zh-CN" onOpenResource={ignoreResourceOpen}
                                        onLoadResourcePreview={loadDefectResourcePreview}/>
            </section>
            <section className="qa-implementation-panel qa-defect-transcript" data-testid="task-push-first-frame">
                ...
                <output
                    data-testid="task-push-resource-state">{taskPushResourceProjected ? '权威资源已到达' : '仅本地提交快照'}</output>
                <ConversationTranscript state={projectedTaskPushState} language="zh-CN"
                                        onLoadResourcePreview={loadDefectResourcePreview}/>
            </section>
        </main>
    );
...
function MotionPreview(props: { dark?: boolean }) {
    ...
    return (
        <section className={`macos-ai-app session-codex-parity-v1 qa-motion-theme ${theme}`}
                 data-testid={props.dark ? 'motion-dark' : 'motion-light'}>
            <header>
...
function SendScrollPreview() {
    ...
    useLayoutEffect(() => {
        ...
        if (returnLatestButton) buttonObserver?.observe(returnLatestButton, {
            attributes: true,
    ...
    return (
        <section ref={previewRef} className="qa-motion-send-preview session-codex-parity-v1"
                 data-testid="send-scroll-preview" data-narrow={narrow || undefined}>
            <div>
                <div>
                ...
                <div className="qa-motion-fixture-actions">
                    ...
                    <button type="button" data-testid="send-scroll-assistant" onClick={appendAssistantMessage}
                            disabled={alternateConversation}>
                        新增回答
                    </button>
                    <button type="button" data-testid="send-scroll-growth" onClick={toggleDelayedGrowth}
                            disabled={alternateConversation}>
                    ...
                    <button type="button" data-testid="send-scroll-switch"
                            onClick={() => setAlternateConversation((value) => !value)}>
            ...
            <div className="qa-send-transcript ai-workspace">
                <ConversationTranscript key={activeState.conversationId} state={activeState} language="zh-CN"
...
function HistoryPagingPreview() {
    ...
    return (
        <section ref={previewRef} className="qa-motion-send-preview session-codex-parity-v1"
                 data-testid="history-paging-preview">
            <div>
                <h3>向上读取历史消息</h3>
            ...
            <output data-testid="history-paging-counters">
                请求 {requestCount} 次 · 切回 {reopenCount} 次 · 提示挂载 {statusMountCount} 次 ·
                锚点漂移 {anchorDrift === null ? '待触发' : `${anchorDrift.toFixed(1)}px`}
            </output>
            <output data-testid="history-paging-events">{eventLog}</output>
            <output data-testid="history-range-evidence">
                同高水位 {historyPagingRangeEvidence.sameHighWater} ·
                新尾页重叠 {historyPagingRangeEvidence.overlappingTail} ·
                范围断开 {historyPagingRangeEvidence.disconnectedTail}
            </output>
            <div className="qa-send-transcript ai-workspace">
                <ConversationTranscript key={transcriptKey} state={state} language="zh-CN"
    ...
    return (
        <section className="qa-motion-send-preview session-codex-parity-v1" data-testid="complete-message-preview">
            ...
            <div className="qa-motion-fixture-actions">
                ...
                <button type="button" data-testid="complete-message-reopen"
                        onClick={() => setTranscriptKey((key) => key + 1)}>
                    切回已补全会话
                </button>
            </div>
            <output data-testid="complete-message-evidence">
                正文请求 {requestCount} 次 ·
                结尾标记 {complete ? '已显示' : '未显示'} · {simulatedFailure ? `系统恢复 ${complete ? '已完成' : '进行中'}` : '正常读取'}
            </output>
            <div className="qa-send-transcript ai-workspace">
                <ConversationTranscript key={transcriptKey} state={state} language="zh-CN"
                                        onLoadV2Content={controller.loadV2Content}/>
            </div>
        </section>
    );
}
...
function ExecutionPhasePreview() {
    ...
    return (
        ...
        >
            ...
            <div className="qa-motion-fixture-actions">
                <button type="button" data-testid="execution-phase-append" onClick={() => setAppended(true)}
                        disabled={appended}>
                    {appended ? '已追加第 41 条操作' : '增量追加第 41 条操作'}
                </button>
                <button type="button" data-testid="execution-phase-complete"
                        onClick={() => setCompleted((value) => !value)}>
                    {completed ? '恢复运行态' : '切换为完成态'}
                </button>
                <button type="button" data-testid="execution-phase-history"
                        onClick={() => setHistoryOnly((value) => !value)}>
        ...
        activeItems: [
            {
                ...
                payload: activeReentryProjection(JSON.stringify({
                    type: 'reasoning',
...
const activeReentryMissingAuthorityState = {
    ...
    snapshot: activeReentryState.snapshot
        ? {
            ...
            turns: activeReentryState.snapshot.turns.map((turn) => (turn.providerTurnId === activeReentryProviderTurnId ? {
                ...turn,
                status: 'waiting'
            } : turn)),
        }
...
export function ActiveTurnReentryQaApp() {
    ...
    return (
        <main className="macos-ai-app zeus-shell session-codex-parity-v1 qa-page theme-light" data-theme="light"
              data-testid="active-reentry-fixture">
            <header className="qa-heading">
            ...
            <nav className="qa-motion-fixture-actions" aria-label="会话切换">
                ...
                <button type="button" data-testid="active-reentry-missing-authority"
                        onClick={() => setSelected('missing')}>
                    模拟问题通道丢失
                </button>
                <output
                    data-testid="active-reentry-selection">{selected === 'active' ? '运行会话已选中' : selected === 'missing' ? '问题通道丢失' : '其他会话已选中'}</output>
            ...
            <section className="qa-implementation-panel qa-defect-transcript" data-testid="active-reentry-transcript">
                {selected === 'active' || selected === 'missing' ? (
                    <div className="ai-workspace">
                        <ConversationTranscript key={`active-${selected}`}
                                                state={selected === 'missing' ? activeReentryMissingAuthorityState : activeReentryState}
                                                language="zh-CN" onInterrupt={() => setStopRequested(true)}/>
                    </div>
                ) : (
                    <article>
                        <h2>其他会话</h2>
...
function BrowserNavigationFailurePreview() {
    ...
    return (
        <section className="qa-motion-theme session-codex-parity-v1 theme-light"
                 data-testid="browser-navigation-failure-preview">
            <header>
            ...
            <div className="ai-workspace" style={{blockSize: 620}}>
                <BrowserWorkspace conversationId="qa-browser-navigation-failure" language="zh-CN"
                                  onClose={() => undefined} onToggleExpanded={() => undefined}
                                  onResetSize={() => undefined} onStageComments={() => undefined}/>
            </div>
        </section>
    );
}
...
function InterruptedProcessPreview() {
    ...
    return (
        <section className="qa-motion-theme session-codex-parity-v1 theme-light"
                 data-testid="interrupted-process-preview">
            <header>
                <strong>重启后最后一轮中断过程</strong>
                <small>编排已收口为
                    interrupted，过程首次打开仍保持展开并自动读取详情；执行期摘要不再作为历史正文保留。</small>
            </header>
            ...
            <div className="qa-motion-transcript ai-workspace" data-testid="interrupted-process-transcript">
                <ConversationTranscript state={interruptedExecutionPhaseState} language="zh-CN"
                                        onLoadTurnProcess={() => setProcessLoadCount((count) => count + 1)}/>
            </div>
        </section>
    );
}
...
function ErrorContractPreview() {
    return (
        <section className="qa-motion-send-preview session-codex-parity-v1" data-testid="error-contract-preview">
            ...
            <div className="qa-motion-fixture-actions">
                <button type="button" data-testid="error-dialog-trigger"
                        onClick={() => reportApplicationError(errorContractFixtures[0], {language: 'zh-CN'})}>
                    记录普通错误
                </button>
...
function CreationFailureExclusivityPreview() {
    return (
        <section className="qa-motion-send-preview session-codex-parity-v1"
                 data-testid="creation-failure-exclusivity-preview">
            <div>
                <h3>创建失败与思考状态互斥</h3>
...
function CreationRetryProgressPreview() {
    return (
        <section className="qa-motion-send-preview session-codex-parity-v1"
                 data-testid="creation-retry-progress-preview">
            <div>
                <h3>创建期自动重试进度</h3>
                <small>只显示当前次数和连接图标，不提前暴露完整错误或手动按钮。</small>
            </div>
            <div className="qa-send-transcript qa-creation-status-narrow ai-workspace">
                <ConversationTranscript state={failedStartingSessionState} language="zh-CN" creationStatus={{
                    state: 'retrying',
                    message: '正在重试',
                    retryAttempt: 3,
                    maxRetries: 5
...
function PlanCustomAnswerProjectionPreview() {
    return (
        <section className="qa-motion-send-preview session-codex-parity-v1"
                 data-testid="plan-custom-answer-projection-preview">
            <div>
...
export function MotionApp() {
    return (
        <main className="macos-ai-app zeus-shell qa-page qa-motion-page">
            ...
            <StopButtonPreview/>
            <section className="qa-motion-theme session-codex-parity-v1 theme-light"
                     data-testid="runtime-details-horizontal">
                <h2>运行详情分区数据表</h2>
                <RuntimeDetails runtime={runtimeDetailsFixture} language="zh-CN" scope="session"/>
            </section>
            <section className="qa-motion-theme session-codex-parity-v1 theme-light"
                     data-testid="runtime-details-unavailable">
...
function SubagentTranscriptPreview() {
    ...
    return (
        <section className="qa-motion-theme session-codex-parity-v1 theme-light"
                 data-testid="subagent-transcript-parity">
...
function ConversationSelectionRecoveryPreview() {
    ...
    const selectedConversation = selected === 'running' ? {
        ...runningConversation,
    ...
    return (
        <section className="qa-motion-theme session-codex-parity-v1 theme-light"
                 data-testid="conversation-selection-recovery">
            <header>
            ...
            {presentation === 'interactive' ?
                <SessionPlanProgress plan={selectionRecoveryPlan} language="zh-CN"/> : null}
        </section>
    );
}
...
export function SessionStylesQaApp() {
    return (
        <main className="macos-ai-app zeus-shell session-codex-parity-v1 qa-page" data-theme="light">
            ...
            <section className="qa-comparison qa-approval-comparison">
                <section className="qa-implementation-panel session-codex-parity-v1"
                         data-testid="recovered-user-input-implementation">
            ...
            <section className="qa-implementation-panel qa-resource-implementation"
                     data-testid="inline-resource-implementation">
                <h2>会话正文：可打开资源与不可用引用</h2>
                <div className="ai-workspace">
                    <ConversationMarkdown text={inlineResourceMarkdown} streamId="qa:inline-resources" phase="final"
                                          language="zh-CN" resources={inlineResourceItems}
                                          onOpenResource={ignoreResourceOpen}/>
                </div>
            </section>
        </main>
    );
