...
export class BrowserHost implements BrowserAutomationPort {
  ...
  private async writeBrowserExport(name: string, bytes: Buffer): Promise<string> {
    ...
    const path = join(directoryPath, sanitizeArtifactName(name, 'browser-export.bin'));
  ...
  private async invokePageAssets(input: BrowserAutomationToolCall, method: string, tab: LiveBrowserTab, args: Record<string, unknown>): Promise<{ contentItems: BrowserAutomationContentItem[]; success: boolean }> {
    if (method === 'list') {
      ...
      const assets = (projection.assets ?? []).flatMap((asset, index) => {
        ...
        return [{ id: `asset-${index}`, url: asset.url, name: sanitizeArtifactName(asset.name, `asset-${index}`), kind, contentType: null }];
      });
    ...
    for (const [index, asset] of selected.entries()) {
      try {
        ...
        const path = join(directoryPath, `${String(index + 1).padStart(3, '0')}-${sanitizeArtifactName(asset.name, asset.id)}`);
...
function googleWorkspaceExportRequest(urlValue: string, type: string): { url: string; extension: string } | null {
  let source: URL;
  try {
