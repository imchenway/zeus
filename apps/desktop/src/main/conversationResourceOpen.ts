...
export async function listConversationResourceOpenTargets(
  request: ConversationResourceRequest,
...
export async function openConversationResource(
  request: OpenConversationResourceRequest,
  services: ConversationResourceOpenServices,
): Promise<OpenConversationResourceResult> {
  const intent = await loadConversationResourceIntent(request, services);
  const available = await listConversationResourceOpenTargets(request, services);
  const preferredTarget = request.target === 'preferred'
    ? preferredOpenTarget(intent, services.getSettings())
    : request.target;
  const preferredAvailable = available.targets.some((candidate) => candidate.id === preferredTarget && candidate.available);
  const target = request.target === 'preferred' && !preferredAvailable
    ? fallbackOpenTarget(intent, available.targets)
    : preferredAvailable
      ? preferredTarget
      : null;
  if (!target) {
  ...
  if (target.startsWith('editor:')) {
    ...
    const targetPath = location?.line
      ? `${file.absolutePath}:${location.line}${location.column ? `:${location.column}` : ''}`
      : file.absolutePath;
...
function fallbackOpenTarget(
  intent: ConversationResourceOpenIntent,
  targets: ConversationResourceOpenTarget[],
): ConversationOpenTarget | null {
  const fallbackOrder: ConversationOpenTarget[] = intent.kind === 'website'
    ? ['zeus_browser', 'system_default', 'copy_link']
    : ['zeus_source', 'system_default', 'file_manager', 'copy_path'];
  return fallbackOrder.find((id) => targets.some((target) => target.id === id && target.available)) ?? null;
}

async function loadConversationResourceIntent(
  request: ConversationResourceRequest,
  services: ConversationResourceOpenServices,
): Promise<ConversationResourceOpenIntent> {
  ...
  const url = new URL(
    `/api/projects/${encodeURIComponent(projectId)}/conversations/${encodeURIComponent(conversationId)}/resources/${encodeURIComponent(resourceId)}/open-intent`,
...
async function editorAvailable(editor: EditorTargetDescriptor, services: ConversationResourceOpenServices): Promise<boolean> {
  const paths = [
    ...editor.appPaths,
    ...editor.appPaths.map((path) => resolve(services.applicationHome, 'Applications', basename(path))),
  ];
