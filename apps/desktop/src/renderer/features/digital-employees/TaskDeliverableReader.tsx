...
export function TaskDeliverableReader(props: {
  ...
  return (
    <ModalPortal rootClassName="task-deliverable-reader-root" backdropClassName="task-work-decision-backdrop" onDismiss={props.onClose} role="dialog" aria-labelledby={titleId}>
      <section className="task-deliverable-reader zeus-solid-form-surface" data-modal-surface="dialog">
        <header>
          <span>
            <small>
        ...
        {revisions.length ? (
          <div className="task-deliverable-comparison-control">
            <span>{zh ? '对照其他修订' : 'Compare revisions'}</span>
            <ZeusSelect
              ariaLabel={zh ? '对照其他修订' : 'Compare revisions'}
              size="compact"
              value={comparison?.id ?? ''}
              onChange={setComparisonId}
              options={[{ value: '', label: zh ? '只看当前成果' : 'Current deliverable only' }, ...revisions.map((item) => ({ value: item.id, label: `${zh ? '修订' : 'Revision'} ${item.version} · ${item.createdAt.slice(0, 10)}` }))]}
            />
          </div>
        ) : null}
...
function TaskDeliverableComparison(props: { taskId: string; deliverable: TaskWorkDeliverableRecord; client: DigitalEmployeeApiClient; language: 'zh-CN' | 'en-US' }) {
  ...
  return (
    <section className="task-deliverable-comparison" aria-label={props.language === 'zh-CN' ? '对照成果' : 'Comparison deliverable'}>
      <h3>
        {props.language === 'zh-CN' ? '对照成果 · 修订' : 'Comparison · Revision'} {props.deliverable.version}
      </h3>
      {state.error ? (
        <div role="alert">
        ...
        <p role="status">{props.language === 'zh-CN' ? '正在读取对照成果…' : 'Loading comparison deliverable…'}</p>
      )}
    </section>
  );
}
