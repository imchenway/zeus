...
type DecisionMode = 'handoff' | 'rework';
...
export function TaskDigitalEmployeePanel(props: TaskDigitalEmployeePanelProps) {
  ...
  async function submitDecision(): Promise<void> {
    if (!props.client || !execution || !currentStage || !candidate || !selectedEmployeeId || !decisionMode) return;
  ...
  return (
    <section className="task-detail-block task-digital-employee-panel task-digital-collaboration" aria-label={zh ? '数字员工协作' : 'Digital employee collaboration'}>
      ...
      {error ? <p className="digital-employee-feedback is-error" role="alert">{error}</p> : null}
          ...
          {candidate && currentStage ? (
            <section className="digital-collaboration-candidate" aria-label={zh ? '待确认阶段交付物' : 'Stage deliverable awaiting confirmation'}>
              ...
              <div className="digital-collaboration-candidate-actions">
                <Button variant="secondary" size="compact" busy={busyAction === `preview:${candidate.id}`} onClick={() => void openDeliverable(candidate)}>{zh ? '预览完整方案' : 'Preview full output'}</Button>
                <Button variant="secondary" size="compact" disabled={props.terminalReadOnly || busyAction !== null} onClick={() => setDecisionMode('rework')}>{zh ? '继续完善' : 'Continue improving'}</Button>
                {finalStage ? (
                  <Button variant="primary" size="compact" busy={busyAction === 'finalize'} disabled={props.terminalReadOnly} onClick={() => void finalize()}>{zh ? '结束协作并进入交付' : 'Finish collaboration and deliver'}</Button>
                ) : (
                  <Button variant="primary" size="compact" disabled={props.terminalReadOnly || busyAction !== null} onClick={() => setDecisionMode('handoff')}>{zh ? '交给另一位数字员工' : 'Hand off to another employee'}</Button>
                )}
              </div>
      ...
      {projection?.blockingReasons.map((reason) => <p key={reason.code} className="digital-employee-boundary-note">{reason.message}</p>)}

      {decisionMode && execution && currentStage && candidate ? (
        <ModalPortal rootClassName="digital-collaboration-modal-root" backdropClassName="digital-collaboration-modal-backdrop" dismissDisabled={busyAction !== null} onDismiss={() => setDecisionMode(null)}>
          <form className="digital-collaboration-dialog zeus-solid-form-surface" role="dialog" aria-modal="true" aria-labelledby="digital-collaboration-dialog-title" onSubmit={(event) => { event.preventDefault(); void submitDecision(); }}>
            <header>
              <span><strong id="digital-collaboration-dialog-title">{decisionMode === 'handoff' ? (zh ? '交给另一位数字员工' : 'Hand off to another employee') : zh ? '继续完善当前阶段' : 'Continue improving this stage'}</strong><small>{currentStage.title} · {candidate.title} v{candidate.version}</small></span>
              <Button variant="secondary" size="compact" disabled={busyAction !== null} onClick={() => setDecisionMode(null)}>{zh ? '取消' : 'Cancel'}</Button>
            </header>
            <label><span>{zh ? '选择数字员工' : 'Choose employee'}</span><EmployeeSelect employees={employees} value={selectedEmployeeId} onChange={setSelectedEmployeeId} language={props.language} /></label>
            {decisionMode === 'rework' ? (
              <label><span>{zh ? '需要完善的内容' : 'What should be improved'}</span><textarea value={reworkReason} onChange={(event) => setReworkReason(event.target.value)} maxLength={4_000} rows={5} autoFocus /></label>
            ...
            <footer><Button variant="primary" size="compact" busy={busyAction === decisionMode} disabled={!selectedEmployeeId || (decisionMode === 'rework' && !reworkReason.trim())} type="submit">{zh ? '确认并开始下一次尝试' : 'Confirm and start next attempt'}</Button></footer>
          </form>
        </ModalPortal>
      ) : null}
      ...
      {preview ? (
        <ModalPortal rootClassName="digital-collaboration-modal-root" backdropClassName="digital-collaboration-modal-backdrop" onDismiss={() => setPreview(null)}>
          <section className="digital-collaboration-preview zeus-solid-form-surface" role="dialog" aria-modal="true" aria-label={preview.title}>
            <header><strong>{preview.title}</strong><Button variant="secondary" size="compact" onClick={() => setPreview(null)}>{zh ? '关闭' : 'Close'}</Button></header>
...
function EmployeeSelect(props: { employees: DigitalEmployeeRecord[]; value: string; onChange: (value: string) => void; disabled?: boolean; language: DigitalEmployeeLanguage }) {
  const zh = props.language === 'zh-CN';
  return <ZeusSelect size="regular" ariaLabel={zh ? '选择项目数字员工' : 'Choose a project digital employee'} value={props.value} onChange={props.onChange} disabled={props.disabled || props.employees.length === 0} options={props.employees.map((employee) => ({ value: employee.id, label: `${employee.name} · ${employee.role}`, searchText: `${employee.domain} ${employee.skillIds.join(' ')}` }))} />;
}
  ...
  return (
    <div className={`task-digital-employee-execution is-${props.execution.status}`}>
      ...
      <span className="task-digital-employee-execution-copy">
        <strong>{props.stage.title} · {props.execution.employeeSnapshot.name}</strong>
        <small>{executionStatusLabel(props.execution.status, props.language)} · {zh ? `第 ${attempt?.attemptNumber ?? props.execution.attempt} 次尝试` : `Attempt ${attempt?.attemptNumber ?? props.execution.attempt}`}</small>
        <small>{props.execution.employeeSnapshot.role} · {props.execution.employeeSnapshot.agentKind} · {props.stage.modelRef || (zh ? '正在解析模型' : 'Resolving model')}</small>
      </span>
      <time dateTime={props.execution.updatedAt}>{formatDateTime(props.execution.updatedAt, props.language)}</time>
      {attempt?.conversationId && props.onOpenConversation ? <Button variant="secondary" size="compact" onClick={() => props.onOpenConversation?.(attempt.conversationId!)}>{zh ? '打开会话' : 'Open conversation'}</Button> : null}
    ...
    <ol className="digital-collaboration-timeline" aria-label={zh ? '协作时间线' : 'Collaboration timeline'}>
      {props.stages.map((stage) => (
        <li key={stage.id} className={`is-${stage.status}`}>
          ...
          <div>
            <header><strong>{stage.title}</strong><small>{stageStatusLabel(stage.status, zh)}</small></header>
            <p>{stage.description}</p>
            {stage.attempts.map((attempt) => (
              <span key={attempt.id} className="digital-collaboration-attempt">
                <span><strong>{employeeSnapshotName(attempt.employeeSnapshot, zh)}</strong><small>{zh ? `第 ${attempt.attemptNumber} 次 · ${attempt.status}` : `Attempt ${attempt.attemptNumber} · ${attempt.status}`}</small></span>
                {attempt.conversationId && props.onOpenConversation ? <button type="button" onClick={() => props.onOpenConversation?.(attempt.conversationId!)}>{zh ? '会话' : 'Conversation'}</button> : null}
              </span>
            ))}
            {stage.deliverables.map((deliverable) => (
              <button key={deliverable.id} type="button" className="digital-collaboration-deliverable" onClick={() => props.onOpenDeliverable(deliverable)}>
                <span><strong>{deliverable.title}</strong><small>v{deliverable.version} · {deliverable.status}</small></span><span>›</span>
              </button>
            ))}
          </div>
        </li>
...
function LegacyExecution(props: { execution: DigitalEmployeeExecutionRecord; adoptionAvailable: boolean; busy: boolean; disabled: boolean; language: DigitalEmployeeLanguage; onAdopt: () => void; onOpenConversation?: (conversationId: string) => void }) {
  const zh = props.language === 'zh-CN';
  return (
    <div className="digital-collaboration-legacy">
      ...
      <span>
        {props.execution.conversationId && props.onOpenConversation ? <Button variant="secondary" size="compact" onClick={() => props.onOpenConversation?.(props.execution.conversationId!)}>{zh ? '打开会话' : 'Open conversation'}</Button> : null}
        {props.adoptionAvailable ? <Button variant="primary" size="compact" busy={props.busy} disabled={props.disabled} onClick={props.onAdopt}>{zh ? '将本次输出作为交接起点' : 'Use this output as handoff start'}</Button> : null}
      </span>
    </div>
  );
}
...
function stageStatusLabel(status: TaskStageRecord['status'], zh: boolean): string {
  const labels: Record<TaskStageRecord['status'], [string, string]> = {
    pending: ['待指派', 'Waiting'], ready: ['待启动', 'Ready'], running: ['执行中', 'Running'], awaiting_acceptance: ['待确认', 'Awaiting confirmation'], accepted: ['已接受', 'Accepted'], changes_requested: ['待返工', 'Changes requested'], failed: ['失败', 'Failed'], cancelled: ['已取消', 'Cancelled'], skipped: ['已跳过', 'Skipped'],
  };
