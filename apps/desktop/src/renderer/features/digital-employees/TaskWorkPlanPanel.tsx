            ...
            <div>
              <label>
                ...
                <input
                  ...
                  value={recipeName}
                  maxLength={120}
                  aria-invalid={recipeNameRequired || undefined}
                  aria-describedby={recipeNameRequired ? recipeNameErrorId : undefined}
                  onChange={(event) => {
                    setRecipeName(event.target.value);
                    if (event.target.value.trim()) setRecipeNameRequired(false);
                  }}
                />
                {recipeNameRequired ? (
                  <span id={recipeNameErrorId} role="alert">
                    请填写配方名称，再保存配方。
                  </span>
                ) : null}
              </label>
              <Button
                size="compact"
                variant="secondary"
                disabled={busy || draft.some((stage) => !stage.title.trim() || stage.assignments.some((item) => !item.title.trim() || !item.description.trim()))}
                onClick={() => {
                  if (!recipeName.trim()) {
                    setRecipeNameRequired(true);
                    recipeNameRef.current?.focus();
                    recipeNameRef.current?.scrollIntoView({ block: 'center' });
                    return;
                  }
