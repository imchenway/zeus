...
export const languageCopy = {
  'zh-CN': {
    ...
    taskWorkspace: {
      ...
      taskCreateZentaoLinkHelp: '粘贴链接后自动解析并填入标题、类型、描述和附件；在设置中配置禅道账号后优先通过 REST 接口解析。',
      ...
      taskCreateZentaoApplied: (title: string, attachmentCount = 0, attachmentFailedCount = 0) =>
        `已从禅道解析并填入：${title}${attachmentCount > 0 ? `；已带入 ${attachmentCount} 个附件` : ''}${attachmentFailedCount > 0 ? `；另有 ${attachmentFailedCount} 个附件下载失败` : ''}`,
  ...
  'en-US': {
    ...
    taskWorkspace: {
      ...
      taskCreateZentaoLinkHelp: 'Paste a link to parse and fill the title, type, description, and attachments automatically. After configuring a ZenTao account in Settings, links are parsed through the REST API first.',
      taskCreateZentaoParse: 'Parse and fill',
      taskCreateZentaoParsing: 'Parsing…',
      taskCreateZentaoApplied: (title: string, attachmentCount = 0, attachmentFailedCount = 0) =>
        `Filled from ZenTao: ${title}${attachmentCount > 0 ? `; imported ${attachmentCount} attachment${attachmentCount === 1 ? '' : 's'}` : ''}${attachmentFailedCount > 0 ? `; ${attachmentFailedCount} attachment${attachmentFailedCount === 1 ? '' : 's'} failed to download` : ''}`,
