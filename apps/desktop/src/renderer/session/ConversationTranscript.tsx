...
export function ConversationTranscript(props: ConversationTranscriptProps) {
  ...
  const lastAssistantKey = [...items].reverse().find((entry) => isFinalAnswerItem(entry))?.key ?? [...items].reverse().find((entry) => itemRole(entry) === 'assistant')?.key;
            ...
            turnRows.map((row) => {
                ...
                if (!turn) {
                  return (
                    <Fragment key={row.key}>
                      {row.rows.map((child) => (
                        <Fragment key={child.key}>{renderTranscriptRow(child, transcriptRowRenderOptions(props, items, showThinking, lastUserKey, lastAssistantKey, true))}</Fragment>
                      ))}
                    </Fragment>
                  );
                }
                ...
                return (
                  <Fragment key={row.key}>
                    <SessionTurnDuration turn={turn} requests={props.state.pendingRequests} language={props.language}>
                      {row.rows.map((child) => (
                        <Fragment key={child.key}>{renderTranscriptRow(child, transcriptRowRenderOptions(props, items, showThinking && props.state.activeTurnId === row.turnId, lastUserKey, lastAssistantKey, true))}</Fragment>
                      ))}
                      {showThinking && props.state.activeTurnId === row.turnId ? <TranscriptThinking language={props.language} /> : null}
                    </SessionTurnDuration>
                    {renderMarkers ? renderTurnArtifacts(row.turnId, props, lastItemKeyByTurn[row.turnId]) : null}
                  </Fragment>
                );
              }
              ...
              return (
                <Fragment key={row.key}>
                  ...
                  {renderTranscriptRow(row, transcriptRowRenderOptions(props, items, showThinking, lastUserKey, lastAssistantKey, false))}
                  {closesVisibleTurn ? renderTurnArtifacts(lastRowItem.turnId, props, lastRowItem.key) : null}
                </Fragment>
              );
            })
...
interface TranscriptRowRenderOptions {
  ...
  lastAssistantKey: string | undefined;
  insideWork: boolean;
}
...
function transcriptRowRenderOptions(
  ...
  return { props, items, showThinking, lastUserKey, lastAssistantKey, insideWork };
}
...
function renderTranscriptRow(row: TranscriptRow, options: TranscriptRowRenderOptions): ReactNode {
  ...
  return (
    <ThreadItemView
      ...
      showAssistantActions={!options.insideWork && row.item.key === options.lastAssistantKey && !options.showThinking}
