...
export function PlanImplementationRequestSurface(props: {
  ...
  return (
    <section className="session-question-panel session-plan-implementation-request" aria-busy={props.busy || undefined} data-error={Boolean(props.error) || undefined} data-request-id={props.request.id}>
      <header>
        <strong>{zh ? '实施此计划？' : 'Implement this plan?'}</strong>
        <button type="button" aria-label={zh ? '关闭' : 'Close'} onClick={() => void props.onRespond(props.request.id, { action: 'dismiss' })} disabled={props.busy}>
          <X aria-hidden="true" />
        </button>
      </header>
      <div className="session-question-options">
        <button ref={primaryRef} type="button" className="session-question-option is-primary" onClick={() => void props.onRespond(props.request.id, { action: 'implement' })} disabled={props.busy}>
      ...
      <footer>
        <span />
        <button type="button" onClick={() => void props.onRespond(props.request.id, { action: 'dismiss' })} disabled={props.busy}>
          {zh ? '跳过' : 'Skip'}
        </button>
      </footer>
    </section>
