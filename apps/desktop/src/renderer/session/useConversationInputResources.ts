...
export function useConversationInputResources(
  options: UseConversationInputResourcesOptions,
): ConversationInputResourceHandlers {
  ...
  const addFiles = useCallback((files: File[], source: 'paste' | 'drop') => {
  ...
  const materializeLongText = useCallback((text: string, selection: TextSelection) => {
    void runResourceOperation(async () => {
      ...
      try {
        const attachments = await bridge([
          {name: 'Pasted text.txt', type: 'text/plain', text, source: 'paste', kind: 'pasted_text'},
        ]);
  ...
  const handlePasteShortcut = useCallback((event: KeyboardEvent<HTMLTextAreaElement>) => {
  ...
  const restorePastedText = useCallback((attachment: NativeConversationAttachment) => {
    ...
    insertText(
      ...
      textarea ? currentSelection(textarea) : {start: latest.current.text.length, end: latest.current.text.length},
    );
