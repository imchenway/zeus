...
import type { CodexLocalUsageDay, CodexLocalUsageGroup, CodexOfficialUsageSnapshot, CodexUsageAnalyticsSnapshot, CodexUsageRange, CodexUsageSummarySnapshot } from '@zeus/shared';
...
export function CodexUsageSettingsPane(props: { client: UsageClient | null; language: Language; refreshRevision: number }) {
  ...
  return (
    <section className="settings-product-pane codex-usage-settings" aria-label={copy.title}>
      ...
      {error ? (
        <p className="codex-usage-state is-error" role="alert">
          {error}
        </p>
      ) : null}
      {snapshot ? (
...
export function ZeusUsageTitlebar(props: { client: UsageClient | null; language: Language; refreshRevision: number; onOpenFull: () => void; onOpenConfiguration: () => void }) {
  const copy = text[props.language];
  ...
  useEffect(() => {
    if (!open || props.refreshRevision === 0) return;
    const timer = window.setTimeout(() => void load(), 180);
    return () => window.clearTimeout(timer);
  }, [load, open, props.refreshRevision]);

  return (
    <div className="zeus-usage-titlebar">
      <button ref={buttonRef} type="button" className="zeus-usage-titlebar-button" aria-haspopup="dialog" aria-expanded={open} onClick={() => setOpen((current) => !current)}>
        Zeus
      </button>
      {open ? (
        <div ref={popoverRef} className="zeus-usage-popover" role="dialog" aria-label={copy.official} tabIndex={-1}>
          <header>
            <span>
              <strong>Zeus</strong>
              <small>{snapshot?.official.planType ?? copy.unavailable}</small>
            </span>
            <button type="button" onClick={close} aria-label={props.language === 'zh-CN' ? '关闭' : 'Close'}>
              ×
            </button>
          </header>
          {loading && !snapshot ? <p role="status">{copy.loading}</p> : null}
          {error ? (
            <p className="is-error" role="alert">
              {error}
            </p>
          ) : null}
          {snapshot ? <UsagePopoverBody snapshot={snapshot} language={props.language} /> : null}
          <footer>
            {snapshot?.official.state === 'signed_out' ? (
              <button
                type="button"
                onClick={() => {
                  close();
              ...
              >
                {props.language === 'zh-CN' ? '前往配置' : 'Open configuration'}
              </button>
            ) : null}
            <button
              type="button"
            ...
            >
              {props.language === 'zh-CN' ? '查看完整统计' : 'View full statistics'}
            </button>
          </footer>
        </div>
      ) : null}
...
function UsagePopoverBody(props: { snapshot: CodexUsageSummarySnapshot; language: Language }) {
  const { snapshot } = props;
  const copy = text[props.language];
  const mostUrgent = [...snapshot.official.rateLimitWindows].sort((left, right) => left.remainingPercent - right.remainingPercent)[0];
  return (
    <div className="zeus-usage-popover-body">
      {snapshot.official.stale ? <p className="codex-usage-stale">{copy.stale}</p> : null}
      {snapshot.official.state === 'unsupported' ? <p>{copy.unsupported}</p> : null}
      {snapshot.official.state === 'signed_out' ? <p>{copy.signedOut}</p> : null}
      <MetricGrid
        language={props.language}
        items={[
          [props.language === 'zh-CN' ? '今日官方 Token' : 'Official today', formatTokens(snapshot.officialTodayTokens, props.language)],
          [props.language === 'zh-CN' ? '近 7 日官方 Token' : 'Official 7 days', formatTokens(snapshot.officialSevenDayTokens, props.language)],
          [
            props.language === 'zh-CN' ? '配额剩余' : 'Quota remaining',
            mostUrgent ? `${formatPercent(mostUrgent.remainingPercent / 100, props.language)}${mostUrgent.resetsAt ? ` · ${formatReset(mostUrgent.resetsAt, props.language)}` : ''}` : copy.unavailable,
          ],
          [props.language === 'zh-CN' ? '本地缓存命中率' : 'Local cache hit rate', formatPercent(snapshot.localSevenDay.cacheHitRate, props.language)],
          ['Credits', formatEstimate(snapshot.localSevenDay.estimatedCredits, 'credits', props.language)],
          [props.language === 'zh-CN' ? 'API 等价' : 'API equivalent', formatEstimate(snapshot.localSevenDay.apiEquivalentUsd, 'usd', props.language)],
        ]}
      />
      <small>{formatUpdatedAt(snapshot.updatedAt, props.language)}</small>
    </div>
  );
}
...
function OfficialOverview(props: { snapshot: CodexOfficialUsageSnapshot; language: Language }) {
  ...
  if (props.snapshot.state === 'unavailable' && !props.snapshot.fetchedAt) return <p className="codex-usage-state">{props.snapshot.error ?? copy.unavailable}</p>;
  return (
    <>
