...
export function ModelConnectionsSettingsPane(props: {
  ...
  async function refreshModels(): Promise<void> {
    ...
    try {
      ...
      setMessage(`${formatVisibleApplicationError(error, zh ? 'zh-CN' : 'en')} ${zh ? '模型目录未验证；可以重试获取，也可以手工添加模型并保存。' : 'The model catalog is unverified; retry or add and save a model manually.'}`);
    } finally {
      setStatus('idle');
    }
  }
