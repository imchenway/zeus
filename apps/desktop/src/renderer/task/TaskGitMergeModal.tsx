  ...
  useEffect(() => {
    ...
    void Promise.all([props.client.loadTaskGitWorkspaces(props.task.id), props.client.loadTaskIntegrations(props.task.id)])
      .then(([workspaceSnapshot, integrationSnapshot]) => {
        ...
        const firstWorkspace = workspaceSnapshot.items.find((workspace) => workspace.id === conflicted?.workspaceId) ?? workspaceSnapshot.items.find((workspace) => workspace.state !== 'discarded') ?? workspaceSnapshot.items[0];
