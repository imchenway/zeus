...
export function createTaskModelPushPendingState(input: { task: TaskRecord; projectName: string; request: StartTaskModelPushRequest; form: TaskModelPushForm; prompt: string; now?: string }): TaskModelPushPendingState {
...
export function TaskModelPushPendingWorkspace(props: { language: 'zh-CN' | 'en-US'; pending: TaskModelPushPendingState; onRetry: () => void }) {
  ...
  return (
    ...
    >
      <header className="session-thread-header" data-motion-title="entered">
        ...
        <span className={`session-thread-status session-thread-status-${failed ? 'error' : 'busy'}`} role={failed ? 'alert' : 'status'} aria-live={failed ? 'assertive' : 'polite'}>
