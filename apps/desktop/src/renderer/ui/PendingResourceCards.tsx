...
export function PendingResourceCards(props: PendingResourceCardsProps) {
  ...
  return (
    <ul className={className} aria-label={props.language === 'zh-CN' ? '待提交资源' : 'Pending resources'}>
...
function PendingResourceCard(props: Omit<PendingResourceCardsProps, 'resources' | 'className'> & {resource: PendingResourceCardItem}) {
  ...
  const visual = (
    <span className="pending-resource-visual" data-color-family={resourceColor} aria-hidden="true">
      {props.resource.kind === 'image' && previewUrl && !previewFailed ? (
  ...
  return (
    ...
    >
      {props.onActivate ? (
        <button
          type="button"
          className="pending-resource-activate"
          aria-label={`${pendingResourceOpenLabel(props.resource.kind, props.language)}: ${props.resource.name}`}
        ...
        <span className="pending-resource-copy">
          ...
          <span className="pending-resource-meta">
            ...
            {props.resource.kind === 'pasted_text' && props.resource.restorable && props.onRestoreText ? (
              <button
                type="button"
                className="pending-resource-restore"
                disabled={props.disabled}
                onClick={() => props.onRestoreText?.(props.resource)}
              >
...
function pendingResourceExtension(name: string): string {
  const extension = name.trim().split('.').at(-1)?.replace(/[^a-z0-9]+/giu, '').toLocaleLowerCase() ?? '';
...
function pendingResourceTypeLabel(resource: PendingResourceCardItem, language: 'zh-CN' | 'en-US'): string {
  ...
  if (resource.kind === 'pasted_text') {
    const textLabel = resource.characterCount === undefined
      ? 'TXT'
      : language === 'zh-CN'
        ? `TXT · ${resource.characterCount.toLocaleString()} 字符`
        : `TXT · ${resource.characterCount.toLocaleString()} chars`;
    return textLabel;
  }
