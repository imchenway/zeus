...
# ZEUS-0117 文件编辑审批卡片设计验收

## 对比对象与状态

- 来源视觉：`/Users/david/.zeus/artifacts/task-attachments/1786068009652-add7f95d-1ca9-40ef-80d1-7e9c26f723be-Codex 2026-08-07 09.59.54.png`，原始尺寸 `1524 × 418`。
- 实现截图：`docs/evidence/zeus-0117-approval-preview.jpeg`，隔离 `Zeus Test.app` 窗口尺寸 `1196 × 768`。
- 同屏比较：`docs/evidence/zeus-0117-approval-comparison.png`；来源缩放为 `762 × 209`，实现截取审批区域后归一到相同宽度。
- 状态：浅色、简体中文、文件编辑审批、权限模式为只读。来源图为授权菜单展开态；实现静态证据为菜单收起态，展开态通过测试包辅助功能树单独复核。

## 必查视觉面

- 字体：动作身份、问题、路径与按钮形成四级层级；路径目录弱化、文件名加深，没有异常换行或裁切。
- 间距：卡片、问题、路径区域和底部动作形成稳定的纵向节奏；最终截图没有底部裁切。
- 形状与颜色：卡片、路径区域、拒绝按钮、主按钮和分段展开按钮使用现有语义变量；分段按钮边界连续，没有圆形按钮脱节。
- 图标与资产：使用项目现有 Phosphor 铅笔、信息与上下箭头图标，没有手工 SVG、字符图标或伪造素材。
- 文案：产品主体使用 Zeus；保留真实“只读”权限模式；没有照抄与现有键盘规则冲突的 `Escape` 提示。

## 交互与安全核对

- 展开菜单包含“允许一次”和“允许所有编辑”；后者的信息说明为“本会话仅对已审批文件免重复询问；新文件仍可能再次申请”。
- 文件目标来自真实 `fileChange` 条目；路径不可见时允许类决定不会出现，审批保持失败关闭。
- 拒绝按钮保留初始键盘焦点。对照图中的蓝色焦点环属于可访问性状态，不是布局漂移。
- 未选择任何授权决定；预览结束后已关闭隔离测试应用。

## 迭代记录

1. 首次预览发现预览壳层盒模型导致底部裁切，已修正壳层尺寸计算。
2. 首次预览发现主按钮和展开按钮之间存在间隙，已收紧分段控件边界与圆角。
3. 修正后重新截图并放入同一比较输入，未发现遗留的 P0、P1 或 P2 视觉问题。

## 验收边界

- 组件级视觉和菜单结构通过。
- 隔离测试包中的 Codex 尚未登录，真实 Provider 文件修改请求无法发出；因此本结论不等同于 Provider 往返验收。
- Zeus 内置浏览器本轮未能建立可用标签页，没有改用外部 Playwright；Electron 视觉核对由 Computer Use 完成。

final result: passed

# ZEUS-000049 选项描述与行高设计校验

## 校验对象

- 参考图：`/Users/david/Library/Application Support/@zeus/desktop/conversation-attachments/1785825063454-1f8f6592-c3b2-40f4-8b2d-8cfd2aa0082b-image.png`
- 实现截图：`/tmp/ZEUS-000049-implementation-final-44.png`
- 并排对比图：`/tmp/ZEUS-000049-comparison-final-44.png`
- 真实应用：`dist/test/mac-arm64/Zeus Test.app`
- 应用身份：`dev.hypha.zeus.test`

## 对比方法

- 参考图原始尺寸为 `1696 × 832`，截取面板区域 `1474 × 469` 后归一到 `737 × 235`。
- Zeus Test 截图视口为 `1226 × 768`，截取面板区域 `538 × 173` 后按相同面板宽度归一到 `737 × 237`。
- 参考图与实现图放入同一张并排图片后检查，不分别凭印象判断。
- 目标依赖 Electron 执行宿主和真实 `request_user_input` 状态，因此使用 Computer Use 检查打包应用；普通网页浏览器不能完整复现这条产品链路。

## 检查结果

- 字体：问题标题、答案主描述、推荐标记和补充说明的字号、粗细层级一致，没有标题下沉或主描述偏上。
- 间距：四行视觉中心在归一图中分别对齐；最终差异不超过约 `1px`，面板总高度相差约 `2px`。
- 布局：主描述、推荐标记和说明按同一基线自然接排；第二、三项不保留推荐空槽。
- 底部输入：与答案文字起点一致，禁用态无边框、无阴影、无拖拽手柄，跳过按钮与输入行居中。
- 颜色与形状：选中背景、弱化文字、编号圆、面板边框、圆角和箭头均沿用 Zeus 现有语义色与 Phosphor 图标，和参考图没有可行动差异。
- 文案：参考图使用 `ChatGPT`，Zeus 使用产品既有术语 `Codex`；这是产品命名差异，不是视觉偏差。
- 交互：真实请求可跳过；自定义输入可选中并增高，输入事件不再触发错误边界。

## 迭代记录

1. 固定说明列和答案等高网格与参考图不符，撤销固定列与最长行联动。
2. `40px` 行高过紧，底部禁用输入仍受全局控件样式影响；补齐无边框覆盖。
3. `48px` 行高让相邻行中心比参考图多约 `4px`；缩减到 `44px`。
4. `44px` 最小行高下，标题、四行中心和面板总高度与参考图对齐，没有剩余的 P0、P1 或 P2 视觉问题。
...
- 视觉真相：`/Users/david/.zeus/artifacts/task-attachments/1785901195512-6aea1ece-4e4f-40c2-a74e-0b55fae981d6-pasted-task-screenshot-1785901195507.png`
- 最终实现截图：`docs/evidence/ZEUS-0074_快捷操作卡片_最终测试包.jpg`
- 最终并排证据：`docs/evidence/ZEUS-0074_参考图与最终测试包对比.png`
- 状态：浅色主题、任务会话、卡片展开、Git 状态已加载；参考图有两个来源，测试会话为真实来源空态。

## 视口与归一化

- 参考图：666 × 744 像素；卡片聚焦区域约 600 × 690 像素。图片没有可验证的 CSS 视口或设备密度元数据，按可见像素作为视觉真相。
- 实现图：Sky 截图 1162 × 768 像素；应用恢复窗口为 1240 × 820 CSS 像素，卡片 CSS 宽度为 360 像素，聚焦区域约 338 × 328 截图像素。
- 并排图：分别裁切卡片区域并等高缩放到 680 像素，中间加入 24 像素白色间隔。该归一化用于比较层级、间距、颜色、图标和文案，不把截图密度造成的锐度差异记为实现缺陷。
- 全景证据：`docs/evidence/ZEUS-0074_快捷操作卡片_最终测试包.jpg`，用于确认卡片锚定在会话右上角且不挤压对话与输入框。
- 聚焦证据：`docs/evidence/ZEUS-0074_参考图与最终测试包对比.png`，卡片内文字和图标已经足够清晰，不需要继续拆分局部区域。

## Findings

- 没有遗留可执行的 P0、P1 或 P2 问题。
- [P3] 非空来源视觉状态未在本次真实包中出现
  - 位置：快捷卡片“来源”区。
  - 证据：参考图显示两个来源；隔离测试会话没有结构化来源，因此最终截图显示真实空态。
  - 影响：不影响当前卡片的核心使用，也不代表来源数据被伪造；只是本轮 GUI 证据没有覆盖非空列表。
  - 后续：有天然带结构化来源的测试会话时，再补一次非空列表截图即可；不应为视觉验收改写正式或共享测试数据。

## 必查表面

- 字体与排版：沿用 Zeus 的系统字体栈；标题、主标签、辅助说明和 Git 增删数字形成与参考图一致的四级层次，长目录与分支使用单行截断，没有溢出。
- 间距与布局：信息顺序、图标列、正文列、尾部状态列、来源分隔线和圆角浮层均与参考图一致；360 像素宽度为长分支与目录保留可读空间，属于 Zeus 桌面端适配。
- 颜色与令牌：卡片使用会话现有背景、边线、弱文本、成功和危险色令牌；浅色测试包中对比度、阴影和边界清楚，没有新造固定主题色。
- 图片与资源质量：界面不需要位图素材；所有可见图标来自现有 Phosphor 图标库，没有手绘 SVG、字符图标、CSS 图形或占位图片。
- 文案与内容：保留参考图的“环境信息 / 变更 / 本地 / 提交或推送 / 比较分支 / 来源”结构；补充的目录、来源分支、待提交文件数和比较目标均来自真实会话与任务工作区。
- 状态与可访问性：已观察加载态、就绪态和来源空态；入口与操作均为语义按钮，卡片有对话框标签，`Escape` 关闭后焦点返回入口。Git 写操作没有在验收中触发。
- 视口韧性：卡片使用 `min(360px, 100vw - 32px)` 与视口高度上限；窄窗口入口会收敛为图标。最终 GUI 截图覆盖桌面视口，窄视口和深色主题由现有会话令牌与 CSS 规则覆盖，未单独做 GUI 截图。

## 对比历史

1. 首轮证据：`docs/evidence/ZEUS-0074_参考图与测试包对比.png`。
   - [P2] 卡片右上角使用扳手，而参考图是设置齿轮；当前分支使用 GitHub 标志，而参考图是分支图标，语义和视觉家族不一致。
   - 修复：设置入口改用 `GearSix`，当前分支改用 `GitBranch`；比较分支保留 `GithubLogo`。
2. 修复后重新执行类型检查、构建和测试包打包，并从最终测试包重新截图。
   - 复核证据：`docs/evidence/ZEUS-0074_参考图与最终测试包对比.png`。
   - 结果：齿轮、分支、GitHub 比较图标与参考图语义一致；信息层级、圆角、阴影、分隔线、增删色和操作顺序没有遗留 P0/P1/P2 差异。

## Open Questions

- 无阻断问题。参考图中的来源内容属于运行数据，不应为追求像素相同而写入隔离测试会话。

## Implementation Checklist

- [x] 保持参考图的信息顺序和操作层级。
- [x] 使用真实会话执行上下文与任务工作区状态。
- [x] 复用既有提交、推送、比较和附件安全入口。
- [x] 完成静态门禁、最终测试包运行、键盘关闭和并排视觉复核。
