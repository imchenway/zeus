...
### 10.23 2026-07-21 Codex App 运行过程 UI/UX 语义对齐

用户要求完整吸收四张 Codex App 截图中的运行过程交互，不只做表面样式模仿。本轮将会话过程明确拆成四个相互独立的领域：`Turn`
负责轮次与耗时，`Plan` 负责权威步骤进度，`Activity` 负责工具与技能活动，`Transport` 负责连接恢复。四者不得继续共用伪造的
`N/M`、原始错误字符串或散落工具卡片表达。

#### 真实事实与边界

1. 当前 Codex app-server 的 `turn/plan/updated` 通知提供 `threadId / turnId / explanation / plan[]`，步骤状态为
   `pending / inProgress / completed`；它是计划进度唯一权威来源，不能从命令条数推算。
2. 真实用户库只读采样中，`commandExecution=105`、`reasoning=71`、`agentMessage=37`、`userMessage=21`、`error=10`、
   `fileChange=1`。连续 command / search / file 事件应聚合，但不得丢失底层事件和顺序。
3. 技能名称只在 `commandActions` 指向真实 `SKILL.md` 路径时展示；普通命令文本不能被猜测成技能调用。
4. 断线是 `Transport` 状态，不属于 `Plan`。界面必须显示真实重试次数和下一次退避时间，保留已加载历史，并提供立即重试；不得显示没有事实依据的“正在重新连接
   5/5”。

#### 实现

1. Storage 新增 `conversation_turns.plan_json` 和迁移 `20260721_0005_conversation_turn_plan`；计划更新按 turn 持久化，恢复会话
   snapshot 继续携带权威计划。
2. Coordinator 接收、校验、持久化并广播 `turn/plan/updated`；API snapshot 和 Renderer reducer 同时支持首屏恢复与增量更新。
3. 新增 `SessionActivityGroup`，把连续 command、search、file、tool、image 活动聚成一条可折叠记录；摘要使用真实动作和技能事实，展开后保留每条事件、状态和详情。
4. 新增 `SessionPlanProgress`，以独立有序步骤展示第 N / M 步、pending / in-progress / completed；活动条目不会占用计划步数。
5. 新增 `SessionTurnDuration`，active turn 实时计时，完成 turn 显示实际起止时间差；耗时与工具记录解耦。
6. reasoning / commentary 改为安静但可见的过程文本，不再藏在“显示工作进度”总开关下；直接 command 回退也收敛为紧凑行。
7. 重连与请求失败移入正常文档流，使用本地化主文案、真实 attempt / delay、立即重试和折叠原始详情；取消绝对定位浮层和 raw
   `request timed out` 主文案。
8. 图标统一使用 Phosphor，补齐 `focus-visible`、深色 token、reduced-motion 和小视口回退；未引入新依赖或另一套视觉系统。

#### 自动验证与反向扫描

- 聚焦测试：6 个相关测试文件，`311 passed`。
- 全仓 `pnpm test`：90 个测试文件，`1492 passed / 4 skipped`。
- `pnpm lint`、`pnpm typecheck`、`pnpm format:check`、`git diff --check`：通过。
- storage、local-server、desktop 独立 build：通过；正式 `pnpm package:mac`：通过。
- 反向扫描确认目标源码不再存在 raw `request timed out` 主文案、伪造重连 `N/M`、绝对定位 reconnect、`>_` 字符图标或已废弃“显示工作进度”入口。

#### 正式包与视觉 QA

- 正式产物：`dist/mac-arm64/Zeus.app`、`dist/Zeus-0.1.0-arm64.zip`、`dist/Zeus-0.1.0-arm64.dmg`。
- 正式包通过 `codesign` 磁盘有效性与 Designated Requirement 校验；仍是本机 ad-hoc 签名，没有 Developer ID /
  notarization，不得表述为外部分发已就绪。
- 正式包真实 `tc-app-core` 会话中，AX 树暴露 `工作活动` 和 `已加载 2 个技能` disclosure；展开后可见真实技能名
  `domain-modeling / grilling`、命令、技能读取和完成状态；轮次耗时显示为 `已处理 2m 20s`。
- 正式包截图：`docs/evidence/zeus-codex-process-activity-packaged-20260721.jpeg`；四张参考图与正式包同屏对比：
  `docs/evidence/zeus-codex-process-activity-comparison-all-20260721.png`；详细结论见 `docs/design-qa.md`。
- 当前真实会话没有 plan 通知，本轮也未主动制造网络故障，所以计划与断线无法完成同状态正式包截图；相关持久化、恢复、退避和交互由自动化测试覆盖，未通过
  mock 或修改用户数据库伪造视觉证据。

#### 回滚

可按 `Plan persistence / Activity projection / Duration / Transport recovery UI` 四个切片独立回滚；若回滚 Storage，迁移后的
nullable `plan_json` 可安全保留。不得清洗或改写既有 conversation item、rollout、任务列配置及工作区其他未提交变更。

未执行 git commit、push、merge、revert 或其他历史修改。

`VERIFICATION PASSED: SOURCE, FOCUSED AND FULL TESTS, OFFICIAL PACKAGE, PACKAGED ACTIVITY UI AND ACCESSIBILITY; EXACT PLAN/RECONNECT STATE SCREENSHOTS BLOCKED BY REAL-STATE BOUNDARY`

### 10.24 2026-07-21 活动命令单行摘要与行尾详情箭头

用户以当前 Codex App 为可见参考，要求继续收敛会话页“工作活动”中的命令展示：多行命令在收起态不得换行；详情入口不得再显示“技术详情”文字按钮，而应使用位于摘要末尾的小箭头。

#### 边界与实现

1. 本轮只调整 `Activity` 中的命令执行条目，不改变技能分组、普通消息、计划进度、轮次耗时、底部 composer、持久化协议或 provider
   原始事件。
2. 命令收起态把真实换行和连续空白折叠为单个空格，并使用
   `white-space: nowrap / overflow: hidden / text-overflow: ellipsis` 保持单行；完整原始命令、工作目录和输出仍保留在展开内容中。
3. 完成、运行中和失败状态进入摘要动词，不再另起一行显示“已完成 / Completed”；中文分别使用“已运行 / 正在运行 /
   运行失败”，英文分别使用“Ran / Running / Failed”。
4. 删除可见“技术详情 / Technical details”入口，改为摘要行尾 `CaretDown`；沿用原生 `details / summary`
   键盘语义、可见焦点和展开旋转反馈，reduced-motion 继续由会话页全局规则覆盖。
5. React 组件继续使用派生渲染，不新增本地展开状态、事件监听器或依赖。

#### TDD 与验证

- 新增 Renderer 回归测试，覆盖多行命令收起后成为单行、行尾箭头存在、完整换行命令仍在详情中、旧技术详情文案和独立完成状态消失，以及单行省略
  CSS 契约。
- 红灯阶段已证明旧实现仍生成换行标题、“Completed”和“Technical details”；最小实现后
  `pnpm exec vitest run apps/desktop/test/session-workspace.test.tsx` 为 `67 passed`。
- `pnpm lint`、`pnpm typecheck`、`pnpm format:check`、`git diff --check` 全部通过。
- 全仓 `pnpm test` 为 `90 passed` 测试文件、`1493 passed / 4 skipped`，共 1497 tests。
- 优雅退出旧 Zeus 后执行正式 `pnpm package:mac` 成功；`dist/mac-arm64/Zeus.app` 通过
  `codesign --verify --deep --strict --verbose=2`，结果为 `valid on disk` 且 `satisfies its Designated Requirement`
  。构建只保留既有的单 chunk 超过 500 kB warning。
- 从正式 `dist/mac-arm64/Zeus.app` 重启真实窗口，在 `tc-app-core / 了解发货链路` 会话中展开“已加载 2 个技能”：AX
  树把每条命令暴露为单个 disclosure，摘要为“已运行 …”，不再暴露“技术详情”或独立“已完成”。
- 点击首条命令行尾箭头后，AX 树和真实窗口同时展开完整命令、工作目录与输出；再次收起能力由原生 `details / summary`
  保持。正式包截图确认长命令单行省略、箭头在行尾，未出现横向溢出、旧文字按钮或框架错误覆盖层。
- 内置网页测试器按安全策略拒绝直接访问 Electron `file://` 资源页，因此没有使用浏览器自动化或 CDP
  绕过；正式包交互改由本机应用可访问性接口完成。控制台日志未单独取证，页面身份、非空渲染、无错误覆盖层、收起态和展开态均已在真实正式包验证。

#### 回滚

仅撤销 `SessionActivity.tsx` 的命令摘要结构、`session.css` 的活动命令样式和 `session-workspace.test.tsx`
对应测试；不得删除底层活动事件、命令原文或输出。

未执行 git commit、push、merge、revert 或其他历史修改。

`VERIFICATION PASSED: SOURCE, FOCUSED AND FULL TESTS, OFFICIAL PACKAGE, PACKAGED COLLAPSED/EXPANDED UI AND ACCESSIBILITY`

### 10.25 2026-07-21 会话正文与工作活动冗余空行收敛

用户在正式包截图中标出了消息正文末尾、消息操作栏前后，以及“工作活动”标题、技能名和活动条目之间的多段冗余空白。本轮目标是消除由多层
CSS 间距叠加形成的假空行，让信息连续但仍可辨认；不改变消息内容、活动分组、详情展开语义或 provider 事实。

#### 真实原因与实现边界

1. 消息气泡和 assistant 正文的最后一个 Markdown 块仍继承 `12px` 下边距，视觉上表现为正文末尾空行；现在只对
   `.session-markdown` 的最后一个直接子块归零，不改变段落之间的 `12px` 节奏。
2. transcript 的 `22px` 行间距与消息操作栏预留空间叠加，形成第二段空白；现在 transcript 收敛为 `14px`，操作栏预留调整为与其实际
   `28px` 控件高度和 `5px` 偏移匹配的 `20px`。复制、评价、展开和时间按钮仍保持 `28px` 点击目标。
3. “工作活动”原有标题最小高度、body 外边距和内边距、技能名下边距、列表 `5px` gap、条目最小高度共同叠加；现在分别收敛为 `28px`
   标题、紧凑 body、`3px` 技能名下边距、`1px` 列表间距和 `24px` 条目。
4. 计划进度列表继续保留原有 `5px` gap，本轮没有把活动区的紧凑样式扩散到 Plan；详情仍使用原生 `details / summary`
   ，键盘语义、焦点反馈和完整命令内容不变。

#### TDD 与阶段验证

- 新增 Renderer CSS 契约测试，逐项锁定 transcript、消息操作预留、Markdown 尾块、活动标题/body/技能/list/row/summary
  的间距，并反向确认消息操作按钮仍为 `28px`。
- 红灯阶段 `pnpm exec vitest run apps/desktop/test/session-workspace.test.tsx` 为 `67 passed / 1 failed`，失败点是旧
  transcript 仍为 `gap: 22px`；实现后同一聚焦测试为 `68 passed`。
- `pnpm lint`、`pnpm typecheck`、`pnpm format:check`、`git diff --check` 全部通过；全仓 `pnpm test` 为 `90 passed` 测试文件、
  `1494 passed / 4 skipped`，共 1498 tests。
- 优雅退出旧 Zeus 后执行 `pnpm package:mac` 成功；正式 `dist/mac-arm64/Zeus.app` 通过
  `codesign --verify --deep --strict --verbose=2`，结果为 `valid on disk` 且 `satisfies its Designated Requirement`
  。构建只保留既有的单 chunk 超过 500 kB warning。
- 正式构建产物 `apps/desktop/dist/renderer/assets/index-*.css` 已反向检出 `transcript gap:14px`、Markdown 尾块
  `margin-block-end:0`、activity body 紧凑间距、activity list `gap:1px` 和 summary `min-block-size:24px`
  ，证明这些规则真实进入打包输入，而非只停留在源码。
- 正式 App 已重新启动；两次读取真实窗口都被 macOS 锁屏拒绝，因此本轮不能声称完成了 packaged visual
  QA。该环境缺口不影响源码、自动化测试、构建产物和签名结论，解锁后仍需回到 `tc-app-core / 了解发货链路` 同一会话补最后一张真实窗口证据。

未执行 git commit、push、merge、revert 或其他历史修改。

`VERIFICATION PASSED: SOURCE, FOCUSED AND FULL TESTS, OFFICIAL PACKAGE AND PACKAGED CSS; PACKAGED VISUAL QA BLOCKED BY MACOS LOCK SCREEN`

### 10.26 2026-07-22 PLAN 全链路与 Codex App 交互对齐

用户要求把 Zeus 的 PLAN 模式入口、计划产物、计划实施确认和普通 `request_user_input` 从“协议可用”提升为与当前 Codex App
一致的完整产品闭环，并以三张 Codex App 参考图进行同视口视觉验收。本节同时记录正式包现场暴露的三项后续回归：历史模型消息重复预留操作栏空间、复制图标为手绘近似、计划卡片和实施确认在纵向
flex 中被压缩裁切。

#### 领域边界

1. `PLAN 协作模式` 决定单次提交的 `collaborationMode`，逐次持久化；排队提交保留各自模式，steer 不回写已启动 turn。
2. `计划进度` 继续只表达 `turn/plan/updated` 的步骤状态，不承载计划正文。
3. `计划产物` 是 app-server `type: "plan"` item 的持久化 Markdown 正文；内联计划卡片和右侧工作区共享同一份
   item，不复制成消息或文件。
4. `计划实施请求` 是 Zeus 在成功 PLAN turn 形成非空完成计划后生成的持久请求；它不是 `request_user_input`，只能对当前会话最新
   pending 请求实施、修订或关闭。
5. `普通用户询问` 继续映射 app-server `request_user_input`，保留 provider 问题、选项、Secret 和自动解决语义。

#### 实现

- 会话、提交、队列和 native turn setting 全链路增加 `default / plan`；旧会话依次从最新提交上下文与 `default` 回退。composer
  支持可恢复模式指示器和 `/plan` 切换。
- plan item 流式阶段显示“正在编写计划”，完成后提供 Markdown 卡片、下载 `plan.md`、复制、反馈、折叠和右侧工作区；桌面使用会话内
  50/50 分栏并支持全宽，小视口退化为覆盖页，关闭后回到原消息流位置。
- 新增 `conversation_plan_actions` 与原子 `respondPlanImplementationRequest`。实施会切回 Default、只在消息流显示“是，实施此计划”，完整计划仅进入
  app-server 执行上下文；修订保持 PLAN，跳过/关闭/Esc 只解决请求，不提交 turn。
- 普通问答面板支持多问题逐题、编号、推荐徽标、其他输入、数字键/方向键/Enter/Shift+Enter、草稿恢复、Secret
  脱敏、队首展示、snooze、服务端截止时间和重启恢复后的超时空响应。
- 修复派生 `conversation.plan_implementation_request.changed` 复用 provider generation/sequence 导致 Renderer
  去重丢弃的问题；派生事件改由 Zeus 分配本地序列。
- 修复纵向 transcript 子项默认 `flex-shrink: 1` 导致计划第二步、修订选项和“跳过”被 `overflow: hidden`
  裁切的问题；计划卡片、问答面板和普通消息行统一 `flex: 0 0 auto`。新增阻塞交互时，只有仍在跟随最新消息的会话会滚到底部，主动查看历史时不抢滚动。
- 模型消息操作栏只在当前最后一条 assistant 回复上渲染；历史 assistant 不再生成 `has-message-actions`，因此不再预留 20px
  空白。复制按钮改用现有 Phosphor `Copy`，删除手绘双方框 SVG。
- “返回最新消息”保持固定挂载的绝对定位覆盖层，通过 `opacity / visibility / pointer-events` 切换；自身出现和 hover
  均不再改变滚动区尺寸或 transform，避免 follow/static 状态抖动。

#### 当前验证状态

- 新增和更新 storage、coordinator、API、Reducer、Renderer、滚动控制器与视觉契约测试；聚焦
  `pending-request-surface + session-workspace + session-scroll-controller` 为 `100 passed`，coordinator 为 `68 passed`。
- `autoResolutionMs` 的重启恢复已用同一 app-server generation 下的 local-server handoff 锁定：pending
  请求与服务端截止时间保持不变，replacement coordinator 恢复计时并在到期后发送 `{ answers: {} }`。完整 Electron 退出会终结其拥有的
  app-server generation，仍按既有 fail-closed 边界处理，不能把失效的 provider requestId 伪装成可继续回答的请求。
- 最新全仓 `pnpm test` 为 `90 passed` 测试文件、`1514 passed / 4 skipped`，共 1518 tests；`pnpm typecheck`、`pnpm lint`、
  `pnpm format:check` 与 `git diff --check` 均为 exit 0。
- 最新 `pnpm package:mac` 成功，产物为 `dist/mac-arm64/Zeus.app`、`dist/Zeus-0.1.0-arm64.zip` 与
  `dist/Zeus-0.1.0-arm64.dmg`；构建仍只有既有的单 chunk 超过 500 kB warning，App 通过 ad-hoc `codesign` 磁盘有效性与
  Designated Requirement 校验。
- 已在隔离资料 `/tmp/zeus-plan-qa.vfRzlm` 中验证 PLAN 生成、持久计划恢复、右侧打开/全宽/恢复、计划修订、实施后切回
  Default，以及普通 `request_user_input` 的推荐项、否定输入和跳过；这些证据只能证明交互可用，不能证明视觉与 Codex App 一致。
- 原三组对照将 Retina `@2x` 局部参考与 Zeus 整窗截图直接并排，像素密度、裁剪范围和状态均不一致，因此撤销“同视口”和“无 P0 /
  P1 / P2”结论。原图保留用于追溯，但不再作为通过证据。

#### 2026-07-22 字体与组件尺寸返工

- 按用户要求先不改变整体会话分栏，只校准问答面板、实施确认、内联计划卡片和右侧计划正文的字体与基础组件尺寸。
- 第一轮按 Retina `@2x -> 1x CSS` 估算为圆角 `20px`、标题 `16/22px`、选项行 `44px`、编号圆 `30px`，但用户在真实 Zeus
  窗口复核后确认仍偏大，撤销该轮视觉结论。
...
- `pnpm typecheck`、相关文件 Prettier / ESLint 和 `pnpm --filter @zeus/desktop build` 通过；构建只保留既有的单 chunk 超过
  `500 kB` warning。
- 第一轮隔离源码构建截图 `request-user-input-size-aligned.jpeg` 与 `implementation-size-aligned.jpeg`
  已被用户否决，保留为失败历史证据。第二轮源码测试和桌面构建通过，但 macOS 锁屏阻止了当前窗口截图，不能宣称视觉通过；也没有覆盖用户正在工作的正式
  Zeus 包。
- 用户明确要求打包后，已正常退出旧正式 Zeus 并执行 `pnpm package:mac`。新产物为 `dist/mac-arm64/Zeus.app`、
  `dist/Zeus-0.1.0-arm64.zip` 和 `dist/Zeus-0.1.0-arm64.dmg`；App 深度签名验证、ZIP 解压测试、DMG 校验和均通过，`app.asar`
  已检出本轮 `session-request-user-input-surface` 规则。正式 App 已重新启动；产物为本机 ad-hoc 签名，未宣称 Apple
  notarization。
- 当前仍有 P1：父级会话内容宽度限制了面板实际宽度，说明整体布局尚未对齐；右侧计划工作区同状态实机复验又被 macOS 锁屏中断。完整边界见
  `docs/design-qa.md`。

未执行 git commit、push、merge、revert 或其他历史修改。

`SIZE CONTRACT IMPLEMENTED; LAYOUT PARITY AND SAME-STATE PACKAGED VISUAL QA BLOCKED`
