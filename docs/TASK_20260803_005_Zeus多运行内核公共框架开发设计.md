...
> 后续状态（2026-08-21）：本文记录的“只搭框架、不启动 Pi”是当时里程碑边界。后续 ZARCH-035 已将 Pi SDK 迁入受监督的独立 Provider Worker，并落地版本化 IPC、反向工具 RPC、熔断与显式恢复；当前边界与验收事实以 `docs/architecture/ZARCH-035_Provider_Runtime_Worker隔离.md` 为准。本文其余内容保留为历史设计记录。
