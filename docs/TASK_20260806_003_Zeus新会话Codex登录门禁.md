...
5. 服务端在创建任何 Codex 产品会话前再次读取账号状态，未就绪时返回稳定的 `ZEUS_CODEX_LOGIN_REQUIRED`，并且发生在创建产品会话、任务工作区和首轮提交之前。
...
- `pnpm lint`、`pnpm typecheck`、`pnpm build`、`pnpm package:mac` 分阶段通过；最终只能启动独立身份的 `Zeus Test.app` 做真实交互验收。
...
- 已使用 Zeus 隔离 `CODEX_HOME` 真实验证：账号读取结果为 `requiresOpenaiAuth=true`、`signedIn=false`；浏览器登录能够取得 HTTPS 地址并成功取消，未改写登录状态。
- `pnpm lint`、`pnpm typecheck`、`pnpm build`、`pnpm package:mac` 已通过。
- 测试包为 `dist/test/mac-arm64/Zeus Test.app`，bundle ID 为 `dev.hypha.zeus.test`，`codesign --verify --deep --strict` 已通过。
