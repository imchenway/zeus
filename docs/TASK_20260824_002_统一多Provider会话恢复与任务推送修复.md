...
源码修复、完整正式门禁、独立测试包和隔离历史库 GUI 回归已完成；v0.3.45 已公开发布并安装到 `/Applications/Zeus.app`。发布后使用正式数据和真实 Codex Provider 完成了“历史会话续发、回复、退出、冷启动、再次续发、再次冷启动”的两轮闭环，正式版历史会话发送与恢复已验收。公开资产、签名边界和现场证据见本文末尾。
...
- 本轮正式回归使用 Codex Provider；Pi 和其他外部模型共用的本地接纳、写出水位和恢复边界已由代码门禁与行为探针覆盖，但没有分别执行真实供应商网络续发，不能把 Codex 现场结果外推成所有供应商均已端到端验收。
- Apple Developer ID 签名与公证未配置；v0.3.45 仍是 ad-hoc 签名，必须继续标记 `signed=false`、`notarized=false`。

## 正式发布证据

### 提交与公开发布

- 功能提交：`64d1f14`（`fix(conversation): unify provider recovery and task dispatch`）。
- 发布提交：`e56d7ada19f57da3a8a5a59a1e1e229ade87e47b`；远端 `main`、本地 `main` 与发布提交一致。
- 版本与标签：`v0.3.45`；GitHub Release：<https://github.com/imchenway/zeus/releases/tag/v0.3.45>。
- Release Workflow `32692325666` 已完成且结论为 `success`，head SHA 为发布提交。
- 公开 DMG `Zeus-0.3.45-arm64.dmg` 为 112,201,854 字节，SHA-256 为 `ecfaa3a062b3906071f41e21293bc2fe717d3c5c111806b709bed500c5b589d6`；已从公开地址完整回下载并通过 `hdiutil verify`。
- 公开 manifest 为 1,047 字节，SHA-256 为 `52477aa777eeaac6a11b7584d4da75184e606346f9e7deb19281af97041d0e74`；Homebrew Cask 已同步到 `0.3.45`。
- 发布读取阶段发生一次 TLS 握手超时，发布编排仅按只读退避策略重试；没有重放远端写入。

### 正式安装版升级与真实 Provider 回归

- Homebrew 正式安装版已升级到 `0.3.45`；`/Applications/Zeus.app` 的 `CFBundleShortVersionString` 与 `brew list --cask --versions zeus` 均为 `0.3.45`，`codesign --verify --deep --strict` 通过。签名仍为 ad-hoc，不宣称 Developer ID 或公证。
- 首次冷启动后打开既有历史会话“目标模式执行过程中发送消息全部失败了”，未出现“正在加载会话”空白页或顶部“连接失败”；发送 `v0.3.45 正式版历史会话恢复验证：请只回复“恢复成功”。` 后，本地消息立即出现，真实 Codex Provider 返回“恢复成功”，轮次进入完成态。
- 退出并重启正式 Zeus 后，应用保留退出前阅读位置并显示“返回最新消息”。点击该入口后，上一轮用户消息和“恢复成功”均完整存在；这不是消息丢失，SQLite 中对应 user/assistant 消息、Provider turn/item 与完成态也均已持久化。
- 在这次重启后的同一历史会话继续发送 `v0.3.45 正式版重启后续发验证：请只回复“续发成功”。`，5 秒内返回“续发成功”，没有新增未知写入、连接失败或不可恢复错误。
- 再次完整退出并冷启动正式 Zeus，约 1.2 秒后工作台已可读；打开同一历史会话时没有加载空白或连接失败，“重启后续发验证”和“续发成功”均从持久状态恢复。该闭环覆盖“发送成功、退出、冷恢复、重启后续发、再次退出、再次冷恢复”。
