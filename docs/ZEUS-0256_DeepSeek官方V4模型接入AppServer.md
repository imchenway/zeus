...
- 将任务分支以 merge 方式合入 `main` 的隔离候选现场只出现 `codexNativeConversationCoordinator.ts` 冲突。
- 合并结果同时保留目标分支对“Provider 启动前的直接工作区错误”的安全恢复保护，以及任务分支按模型来源解析并注入 DeepSeek Responses 运行时的恢复能力。
- 该取舍不改变已有 Pi 会话的续接身份，也不在 Responses 配置失效时静默切换运行内核或重放消息。
