...
## im-04 合入 main 冲突候选（2026-08-31）

- 持久分支 `zeus/ZEUS-0392-im-04-merge` 以本地 `main` 的 `4c83e3c9` 为 `HEAD`，合入任务提交 `e6f76ecb` 后完成语义冲突处理；本轮只暂存候选并保留 `MERGE_HEAD`，没有提交、切换分支、更新 `main` 或推送。
- `main` 的 v0.3.83 修复与 im-04 都修改了任务创建 capability。合并结果保留 `task.create`、`task.await_create`、进程恢复和取消等待态，同时纳入 im-04 的首页导航、`task.push_menu`、`task.push_existing` 与任务会话身份校验。三方文本冲突之外，Git 自动保留了两份 `taskCreatePromptView`；候选删除旧的纯标题版本，保留支持标题与附件的新版本，避免重复方法和交互文案倒退。
