...
- 任务分支基线的最终 Test 包为 `dist/test/mac-arm64/Zeus Test.app`，DMG 为 `dist/test/Zeus-Test-0.3.75-arm64.dmg`。
...
### 8.3 本地 merge 交付记录（2026-08-30）

- 本地 merge 候选以来源分支 `main` 的 `0e3ed1ad` 为 `HEAD`，合入任务分支 `zeus/ZEUS-0395-zeus-computer-use-browser-use-codex-01` 的 `53b70bef`；交付保持 `MERGE_HEAD`，不创建提交、不更新 `main`、不 push。
- 仓库内 5 个冲突文件已按三方语义解决：Codex 与 Pi/DeepSeek 同时保留 main 的通用 Plugin Runtime 和任务分支的 Zeus 原生 Browser/Computer Broker；原生 Browser 不恢复旧直连分支；Pi 继续向当前 Provider 传递图片结果，并在持久 JSONL 中只保留受控制品引用。
- 共同修改审计确认：桌面包保留 main 的 `0.3.79` 与任务分支的 Browser 扩展构建步骤；会话头同时保留 main 的 Plugin/发送恢复能力和任务分支的 Computer 立即停止入口；数据布局同时保留 Plugin 数据目录与 Computer、Browser 原生运行目录。其余 38 个任务独占文件的暂存 blob 与任务分支一致。
- 首轮 `pnpm typecheck` 发现 `apps/desktop/src/main/browserHost.ts` 为 4011 行，超过 main 的 4000 行架构门禁。Browser 导出制品命名和 Google Workspace 官方导出地址解析已抽到 `browserArtifactExport.ts`，宿主文件降至 3972 行，没有通过删除空行规避门禁。
- 冻结锁文件离线恢复依赖后，`pnpm typecheck`（含架构治理）、`pnpm lint`、`pnpm build`、`pnpm package:mac` 均通过；build 仍只有既有第三方 PURE 注解和大 chunk 警告。
- 本次 Test 包为 `dist/test/mac-arm64/Zeus Test.app`，DMG 为 `dist/test/Zeus-Test-0.3.79-arm64.dmg`；主 bundle ID 为 `dev.hypha.zeus.test`，Computer Helper bundle ID 为 `dev.hypha.zeus.test.helper.computer`，打包后的深度签名校验通过。
- 本次 merge 交付没有启动 GUI，也没有执行真实 Codex、Pi/DeepSeek、Computer 或 Browser 交互；8.2 的真实运行缺口继续保留，不能用静态检查、构建或打包结果替代。
