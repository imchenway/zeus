...
| `task_work_runs` | 工作管理 | `Z/E` 冻结能力、上下文与一次运行状态 | 不可；Provider 或命令终态不能反推完整冻结边界 | `T3/T4`，随工作项与审计窗口 | `B-DB` | 运行状态机；非终态和结果未知运行禁止删除 | 禁止自动重发；Agent 会话或 Command 运行仅作为证据核对 |
...
| `task_work_decisions` | 工作管理 | `Z/E` 管理者待办、操作身份与决定结果 | 不可；不能从会话回复或外部现场反推 | `T3/T4`，随工作项审计窗口 | `B-DB` | 管理决定状态机；未决决定禁止无痕删除 | 相关运行保持等待或阻塞；未知结果禁止自动重发 |
