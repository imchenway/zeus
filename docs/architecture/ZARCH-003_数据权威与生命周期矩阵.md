...
下面列出当前代码创建的 128 张表。`不可`表示不能从其他本地事实无损重建；`条件`表示只有保留了指定 Provider 历史、资产或源仓库才能重建。SQLite 全库快照会物理包含全部表；标为 `B-NONE` 的表在逻辑导出、分层备份和未来拆库中可以排除。
...
| `task_workflows` | 工作管理 | `Z` 任务阶段编排当前态 | 不可；不能从会话标题或顺序猜测 | `T4`，随任务 | `B-DB` | 用户任务工作流操作；随任务永久删除 | 阶段编排不可用；保留既有会话与资产但不得猜测阶段归属 |
| `task_stages` | 工作管理 | `Z` 阶段定义、冻结配置与验收状态 | 不可 | `T3/T4`，随工作流 | `B-DB` | 阶段状态机；已启动阶段禁止改写冻结配置 | 停止阶段推进；已有会话只能按普通任务会话只读查看 |
| `task_stage_attempts` | 工作管理 | `E` 独立会话、模型配置与上游输入绑定 | 不可；不能从 Provider 终态反推精确接纳边界 | `T3`，随阶段审计窗口 | `B-DB` | 阶段状态机；非终态尝试禁止直接删除 | 该次阶段执行结果保持未知或失败，禁止自动重发并要求对账 |
| `task_stage_deliverables` | 工作管理 | `Z/A` 交付物版本、验收结论与资产摘要引用 | 条件；完整正文必须有对应 Artifact | `T3/T4`，随任务及审计窗口 | `B-BUNDLE` | 用户验收/返工操作；活动版本禁止资产 GC | 保留版本和摘要并标记正文不可用；不得以会话最新回复冒充已验收版本 |
| `task_work_items` | 工作管理 | `Z` 独立工作责任当前态 | 不可；不得从指派次数、会话或岗位名称推断 | `T4`，随任务 | `B-DB` | 工作项状态机；活动或结果未知时禁止直接删除 | 停止相关工作项，保留无关独立工作；不得创建固定阶段替代 |
| `task_work_runs` | 工作管理 | `Z/E` 冻结能力、上下文与一次运行状态 | 不可；Provider 或命令终态不能反推完整冻结边界 | `T3/T4`，随工作项与审计窗口 | `B-DB` | 运行状态机；非终态和结果未知运行禁止删除 | 禁止自动重发；Agent 普通任务会话或 Command 运行用于核对实际执行 |
| `task_work_deliverables` | 工作管理 | `Z/A` 正式产物版本、哈希与验收状态 | 条件；完整正文必须有对应 Artifact | `T3/T4`，随任务及审计窗口 | `B-BUNDLE` | 管理者验收/返工；被引用版本禁止资产 GC | 保留版本与哈希并标记正文不可用，不用会话末条消息代替 |
| `task_work_decisions` | 工作管理 | `Z/E` 交付物验收或返工、命令确认与未知结果处置 | 不可；不能从外部现场反推 | `T3/T4`，随工作项审计窗口 | `B-DB` | 管理决定状态机；未决决定禁止无痕删除 | 相关运行保持等待或阻塞；未知结果禁止自动重发；会话补充信息和审批由 `conversation_server_requests` 独立拥有 |
