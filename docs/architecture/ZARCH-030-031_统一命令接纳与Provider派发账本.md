...
3. change-set 文件操作、message、side-chat、send-now、resume/recover、interrupt 与两类 request response 是九个 External 注册点、十个命令类型。Application 在真实 Provider/Runtime/文件写入前持久化 `external_operation` write marker，并只允许 `failed_before_write`、`explicitly_rejected`、`outcome_unknown_after_write`、`accepted` 四态；unknown 明确阻断自动重发。
