...
async function executeProjectGitActionInternal(cwd: string, action: ProjectGitAction): Promise<ProjectGitActionResult> {
  ...
  const conflictCapable =
    (action.type === 'subtree' && action.operation === 'pull') || action.type === 'continue_integration' || action.type === 'pull' || action.type === 'merge' || action.type === 'rebase' || action.type === 'apply_stash' || action.type === 'revert' || action.type === 'cherry_pick';
...
export async function getProjectGitComparisonDiff(cwd: string, branchName: string, mode: 'current' | 'working-tree'): Promise<GitDiffSummary> {
  ...
  const args = mode === 'working-tree' ? ['-c', 'core.quotePath=false', 'diff', '--no-ext-diff', '--find-renames', revision, '--', '.'] : ['-c', 'core.quotePath=false', 'diff', '--no-ext-diff', '--find-renames', `${revision}..HEAD`, '--', '.'];
