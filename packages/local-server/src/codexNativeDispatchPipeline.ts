...
export function createCodexNativeDispatchPipeline(dependencies: CodexNativeDispatchPipelineDependencies) {
    ...
    async function dispatchSubmissionWithLease(
        ...
        try {
            ...
            const providerBootstrapUtf8Bytes = Buffer.byteLength(JSON.stringify({
            ...
            if (segmentLifecycle?.contextCompactionPlan) {
                await emitPluginCompactionHook({
            ...
            const compiledDispatchContext = await options.compileDispatchContext({
                provider: 'codex',
                conversationId: conversation.id,
                submissionId: submission.id,
                projectId: context.projectId,
            ...
            assertCallerDoesNotOverrideCompiledContext(context.additionalContext);
            ...
            else options.commandDeliveries.markProviderWriteStarted({
