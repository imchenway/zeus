...
export function routeConversationExpertEvent(input: {
  ...
  const type = normalizedConversationEventType(input.type, input.payload);
  ...
  if (!execution || !routedExpertEventTypes.has(type)) return { type, payload: input.payload };
  ...
  if (type.startsWith('conversation.item.') && itemType === 'userMessage') return null;
  if (type.startsWith('conversation.item.') && itemType === 'agentMessage') {
    return {
...
function normalizedConversationEventType(type: string, payload: Record<string, unknown>): string {
  if (type === 'conversation.item.updated') return payload.status === 'in_progress' ? 'conversation.item.delta' : 'conversation.item.completed';
  if (type === 'conversation.provider.settings.updated') return 'conversation.settings.changed';
  if (type === 'conversation.provider.token_usage.updated') return 'conversation.tokenUsage.changed';
  if (type === 'codex.rate_limits.updated') return 'conversation.rateLimits.changed';
  if (type === 'codex.mcp_startup_status.updated') return 'conversation.mcpStartup.changed';
  if (type === 'conversation.submission.steered') return 'conversation.queue.changed';
  return type;
}
