...
export type ConversationWorkExecutionState =
    ...
    | { type: 'outcome_unknown'; code: string; message: string };
...
export function conversationWorkExecutionState(conversation: ZeusConversationRecord, submissions: readonly ZeusConversationSubmissionRecord[]): ConversationWorkExecutionState {
    ...
    if (
        current &&
        (current.submissionOutcome === 'outcome_unknown' ||
            current.pausedReason === 'outcome_unknown' ||
            current.pausedReason === 'recovery_required' ||
            (current.status === 'paused' && current.pausedReason !== 'user_confirmation'))
    ) {
...
function parseSubmissionError(errorJson: string | null, fallbackCode: string, fallbackMessage: string): {
    code: string;
    message: string
} {
