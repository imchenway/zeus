...
function outcomeUnknown(cause: unknown, redactSensitiveText: (value: string) => { text: string }): GitCommandApplicationError {
  ...
  return new GitCommandApplicationError('ZEUS_GIT_COMMAND_OUTCOME_UNKNOWN', `Git command result is unknown after write started: ${detail}`, 409, true, userFacingErrorCause(cause));
}
...
export function gitCommandHttpError(error: unknown): { statusCode: number; payload: { error: string; message: string; recoveryRequired?: true; cause?: UserFacingErrorCause } } | null {
