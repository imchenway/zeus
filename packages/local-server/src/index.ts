...
import {
  ...
  TaskEventRepository,
  TaskIntegrationAiRunRepository,
...
interface BatchTaskWorkspaceBody {
  message?: string;
}

interface BatchTaskWorkspaceResult {
  workspaceId: string;
...
async function createLocalServerWithDatabase(options: CreateLocalServerOptions, db: ZeusDatabase): Promise<FastifyInstance> {
  ...
  const taskIntegrationAiRuns = new TaskIntegrationAiRunRepository(db);
  const taskConflictAiOperations = new Map<string, { integrationId: string; runId: string; submissionId: string; running: boolean; finalizing: boolean }>();
  const taskConflictAiFinalizationQueues = new Map<string, Promise<void>>();
  ...
  function publishNativeConversationEvent(type: string, payload: Record<string, unknown>): void {
    ...
    if ((mappedType === 'conversation.turn.started' || mappedType === 'conversation.turn.completed') && typeof payload.conversationId === 'string') {
      const operation = taskConflictAiOperations.get(payload.conversationId);
      if (operation) {
        if (mappedType === 'conversation.turn.started') operation.running = true;
        else {
          operation.running = false;
          if (payload.status === 'completed') scheduleTaskIntegrationAiFinalization(operation.runId, payload.conversationId);
        }
      }
    }
  ...
  server.post('/api/tasks/:taskId/git-workspaces/commit-all', async (request: FastifyRequest<{ Params: { taskId: string }; Body: BatchTaskWorkspaceBody }>, reply) => {
    const task = tasks.getById(request.params.taskId);
    if (!task) return reply.code(404).send({ error: 'ZEUS_TASK_NOT_FOUND', message: 'Task not found' });
    const commitMessage =
    ...
    const operationResults = await mapTaskRepositoriesWithConcurrency(taskWorkspaces.listByTask(task.id), async (workspace) => {
      ...
      try {
        ...
        return {
          publicResult: { ...base, status: 'succeeded' as const, message: '已提交。', headSha: result.headSha },
          result,
          selectedPaths,
        };
      } catch (error) {
        return {
          publicResult: { ...base, status: 'failed' as const, message: error instanceof Error ? error.message : '任务工作区提交失败。' },
          attempted: true,
        };
      }
    });
    for (const operation of operationResults) {
      ...
      if (operation.publicResult.status === 'succeeded' && operation.result) {
        taskWorkspaces.update(workspace.id, { headSha: operation.result.headSha, state: 'ready', lastError: null });
        recordTaskEvent({
          taskId: task.id,
          eventType: 'task.git_workspace.committed',
          title: '任务分支已提交',
          payload: { workspaceId: workspace.id, branchName: workspace.branchName, batch: true, ...operation.result },
        });
        appendAuditLog({
          actorType: 'local_api',
      ...
      } else if (operation.publicResult.status === 'failed' && operation.attempted) {
        taskWorkspaces.update(workspace.id, { state: 'failed', lastError: operation.publicResult.message });
      }
    }
    await db.save();
    const items: BatchTaskWorkspaceResult[] = operationResults.map((operation) => operation.publicResult);
    return { taskId: task.id, items, summary: summarizeBatchTaskWorkspaceResults(items) };
  });

  server.post('/api/tasks/:taskId/git-workspaces/push-all', async (request: FastifyRequest<{ Params: { taskId: string } }>, reply) => {
    const task = tasks.getById(request.params.taskId);
    if (!task) return reply.code(404).send({ error: 'ZEUS_TASK_NOT_FOUND', message: 'Task not found' });
    const operationResults = await mapTaskRepositoriesWithConcurrency(taskWorkspaces.listByTask(task.id), async (workspace) => {
      const base = {
      ...
      try {
        ...
        return { publicResult: { ...base, status: 'failed' as const, message: error instanceof Error ? error.message : '任务工作区推送失败。' } };
      }
    });
    for (const operation of operationResults) {
  ...
  server.post('/api/tasks/:taskId/git-workspaces/:workspaceId/integrate', async (request: FastifyRequest<{ Params: { taskId: string; workspaceId: string }; Body: StartTaskIntegrationBody }>, reply) => {
    ...
    if (requestedTargetBranch && requestedTargetBranch !== targetBranch) {
      return reply.code(400).send({
        error: 'ZEUS_TARGET_BRANCH_INVALID',
        message: 'Code delivery can only merge into the recorded source branch.',
      });
    }
    ...
    try {
      ...
      targetHeadSha = localTargetHeadSha ?? workspace.sourceHeadSha;
      if (!localTargetHeadSha) targetRef = targetHeadSha;
    } catch (error) {
      return sendTaskGitApiError(reply, error);
    }
  ...
  server.post(
    ...
    async (request: FastifyRequest<{ Params: { taskId: string; integrationId: string }; Querystring: { path?: string }; Body: AssistTaskIntegrationConflictBody }>, reply) => {
      ...
      try {
        ...
        if (!resolved.integration.taskHeadSha) throw nativeApiError('ZEUS_TASK_HEAD_UNAVAILABLE', '当前合入缺少冻结的任务分支提交，无法创建独立 AI 处理现场。');
        const conversationId = randomUUID();
        const runId = `task_integration_ai_${randomUUID()}`;
        createdRunId = runId;
        taskIntegrationAiRuns.create({
          id: runId,
        ...
        const aiIntegration = await startTaskBranchIntegration({
          repositoryPath: resolved.workspace.repositoryPath || resolved.project.localPath,
          projectSlug: resolved.project.slug,
          integrationId: runId,
          targetBranch: resolved.integration.targetBranch,
          targetRef: resolved.integration.targetHeadSha,
          taskBranch: resolved.workspace.branchName,
          mode: resolved.integration.mode,
          commitMessage: `${resolved.task.taskCode}: 合入 ${resolved.workspace.branchName}`,
        });
        if (aiIntegration.state !== 'conflicted' || !aiIntegration.conflictFiles.includes(path)) {
          taskIntegrationAiRuns.update(runId, { integrationPath: aiIntegration.integrationPath, state: 'failed', lastError: '独立 AI 处理现场与当前冲突已经不一致。' });
          throw nativeApiError('ZEUS_TASK_CONFLICT_CHANGED', '当前冲突已经变化，请刷新代码交付页后重试。');
        }
        taskIntegrationAiRuns.update(runId, { integrationPath: aiIntegration.integrationPath, state: 'active' });
        await writeTaskIntegrationDraft(aiIntegration.integrationPath, path, content);
        ...
        const operation =
          modelConversation.agentKind === 'pi'
            ? await piNativeCoordinator.startConversation({
                ...
                conversationTitle,
                cwd: aiIntegration.integrationPath,
                prompt,
            ...
            : await codexNativeCoordinator.startTaskConversation({
                ...
                submissionId,
                projectId: resolved.project.id,
                projectLocalPath: aiIntegration.integrationPath,
                taskId: resolved.task.id,
                workspaceId: resolved.workspace.id,
                ...(resolved.workspace.environmentId ? { environmentId: resolved.workspace.environmentId } : {}),
                conversationTitle,
                taskTitle: resolved.task.title,
                prompt,
                writableRoots: [aiIntegration.integrationPath],
                model: modelId,
        ...
        taskIntegrationAiRuns.update(runId, { submissionId: operation.submissionId, state: 'active' });
        taskConflictAiOperations.set(operation.conversationId, {
          integrationId: resolved.integration.id,
          runId,
        ...
        if (conversationSubmissions.getById(operation.submissionId)?.status === 'completed') {
          scheduleTaskIntegrationAiFinalization(runId, operation.conversationId);
        }
        recordTaskEvent({
          taskId: resolved.task.id,
        ...
        if (createdRunId) {
          const aiRun = taskIntegrationAiRuns.getById(createdRunId);
          if (aiRun && aiRun.state !== 'completed' && aiRun.state !== 'superseded') {
            taskIntegrationAiRuns.update(createdRunId, {
              state: 'failed',
              lastError: error instanceof Error ? error.message : 'AI 冲突处理会话创建失败。',
            });
            await db.save();
          }
        }
  ...
  server.put(
    ...
    async (request: FastifyRequest<{ Params: { taskId: string; integrationId: string }; Querystring: { path?: string }; Body: ResolveTaskIntegrationConflictBody }>, reply) => {
      const resolved = resolveTaskIntegrationRequest(request.params.taskId, request.params.integrationId);
      if ('error' in resolved) return reply.code(resolved.status).send(resolved.error);
      const aiBusy = [...taskConflictAiOperations.values()].some((operation) => operation.integrationId === resolved.integration.id && (operation.running || operation.finalizing));
      if (aiBusy) {
  ...
  server.post('/api/tasks/:taskId/integrations/:integrationId/finalize', async (request: FastifyRequest<{ Params: { taskId: string; integrationId: string } }>, reply) => {
    ...
    const aiBusy = [...taskConflictAiOperations.values()].some((operation) => operation.integrationId === integration.id && (operation.running || operation.finalizing));
    if (aiBusy) {
      return reply.code(409).send({ error: 'ZEUS_TASK_INTEGRATION_AI_BUSY', message: 'AI 正在对应会话中处理本地合入，请先在会话中完成或中断本轮操作。' });
    }
  ...
  server.post('/api/tasks/:taskId/integrations/:integrationId/push', async (request: FastifyRequest<{ Params: { taskId: string; integrationId: string } }>, reply) => {
    const resolved = resolveTaskIntegrationRequest(request.params.taskId, request.params.integrationId);
    if ('error' in resolved) return reply.code(resolved.status).send(resolved.error);
    const { task, project, integration, workspace } = resolved;
    if (integration.state !== 'merged' || integration.targetBranch !== workspace.sourceBranch) {
      return reply.code(409).send({ error: 'ZEUS_TASK_INTEGRATION_NOT_MERGED', message: 'Merge the task branch into its source branch before pushing.' });
    }
    if (!workspace.remoteName) return reply.code(409).send({ error: 'ZEUS_TASK_GIT_REMOTE_UNAVAILABLE', message: 'This repository has no Git remote. Configure a remote before pushing.' });
    try {
      const result = await pushLocalBranch({
        repositoryPath: workspace.repositoryPath || project.localPath,
        remoteName: workspace.remoteName,
        branchName: integration.targetBranch,
      });
      recordTaskEvent({
        taskId: task.id,
        eventType: 'task.git_integration.source_pushed',
        title: '来源分支已推送',
        payload: { integrationId: integration.id, workspaceId: workspace.id, ...result },
      });
      appendAuditLog({
        actorType: 'local_api',
      ...
      await db.save();
      return { integration, workspace, result };
    } catch (error) {
      // 推送失败只属于本次远端动作，不改写已经完成的本地合入事实。
      return sendTaskGitApiError(reply, error);
    }
  });
  ...
  server.post(
    ...
    async (
      ...
      if (!taskId) return reply.code(400).send({ error: 'ZEUS_TASK_ID_REQUIRED', message: 'taskId is required' });
      const task = tasks.getById(taskId);
      if (!task || task.projectId !== project.id) return reply.code(404).send({ error: 'ZEUS_TASK_NOT_FOUND', message: 'Task not found' });
      try {
        const repository = projectRepositories.getById(request.params.repositoryId);
        if (!repository || repository.projectId !== project.id) return reply.code(404).send({ error: 'ZEUS_PROJECT_REPOSITORY_NOT_FOUND', message: 'Project repository not found' });
        ...
        if (projectRelativePath === '..' || projectRelativePath.startsWith(`..${sep}`) || isAbsolute(projectRelativePath)) {
          return reply.code(409).send({ error: 'ZEUS_PROJECT_REPOSITORY_UNAVAILABLE', message: 'Project repository path is outside the project directory.' });
        }
        ...
        if (!repositoryContext.isRepository || resolve(repositoryContext.topLevel) !== resolve(repository.localPath)) {
          return reply.code(409).send({ error: 'ZEUS_PROJECT_REPOSITORY_UNAVAILABLE', message: 'Project repository is unavailable.' });
        }
        ...
        return await resolveTaskPushRepositoryCapability(project, task, repository, true, { context: repositoryContext, clean });
      } catch (error) {
        return sendNativeConversationApiError(reply, error);
      }
    },
    ...
    const nativeUserMessageClientIds = new Set(
      conversation.messages
        ...
        .map((message) => {
          ...
          return message.clientMessageId ?? (typeof metadata.clientUserMessageId === 'string' && metadata.clientUserMessageId.trim() ? metadata.clientUserMessageId : null);
        })
        ...
        const userMessageProjection =
          message.role === 'user' && message.source === 'codex_native'
            ? resolveNativeUserMessageSubmission({
                submissions,
                providerClientId: clientMessageId,
                providerTurnId: message.providerTurnId,
                existingMessage: { clientMessageId },
                existingClientMessageIds: new Set([...nativeUserMessageClientIds].filter((value) => value !== clientMessageId)),
              })
            : null;
  ...
  function taskConflictIntegrationForConversation(conversation: ZeusConversationRecord): ZeusTaskIntegrationRecord | null {
    const aiRun = taskIntegrationAiRuns.getByConversationId(conversation.id);
    if (aiRun) return taskIntegrations.getById(aiRun.integrationId) ?? null;
  ...
  function resolveNativeConversationExecutionRoot(conversation: ZeusConversationRecord): string | null {
    ...
    if (conversation.taskId) {
      const aiRun = taskIntegrationAiRuns.getByConversationId(conversation.id);
      if (aiRun) return (aiRun.state === 'active' || aiRun.state === 'failed') && aiRun.integrationPath && existsSync(aiRun.integrationPath) ? resolve(aiRun.integrationPath) : null;
  ...
  async function ensureNativeConversationExecutionContext(input: { conversationId: string; mode: 'reconcile' | 'submit' | 'dispatch' | 'recover_queue' | 'restore' }): Promise<{ projectLocalPath: string; writableRoots: string[] } | null> {
    ...
    const promise = (async () => {
      ...
      const aiRun = taskIntegrationAiRuns.getByConversationId(conversation.id);
      if (aiRun?.integrationPath && existsSync(aiRun.integrationPath)) {
        if (aiRun.state !== 'active' && aiRun.state !== 'failed') {
          throw nativeApiError('ZEUS_TASK_INTEGRATION_AI_RUN_CLOSED', '这条冲突处理会话已经完成或失效，只保留历史记录，不能继续修改工作区。');
        }
        const latestSubmission = conversationSubmissions.listByConversation(conversation.id).at(-1);
        const operation = taskConflictAiOperations.get(conversation.id);
        if (!operation) {
          taskConflictAiOperations.set(conversation.id, {
            integrationId: aiRun.integrationId,
            runId: aiRun.id,
        ...
        return { projectLocalPath: resolve(aiRun.integrationPath), writableRoots: [resolve(aiRun.integrationPath)] };
      }
      if (aiRun) throw nativeApiError('ZEUS_TASK_INTEGRATION_PATH_UNAVAILABLE', '这条冲突处理会话的独立现场已经回收，不能退回普通任务工作区继续执行。');
  ...
  interface SynchronizedTaskRepository {
    record: ZeusProjectRepositoryRecord;
    discovered: DiscoveredGitRepository;
  }

  /** Worktree 仓库属于本次推送的实时事实，不再要求用户在项目设置中维护清单。 */
  async function synchronizeDiscoveredProjectRepositories(project: ZeusProjectRecord): Promise<SynchronizedTaskRepository[]> {
    ...
    await db.save();
    const discoveredByPath = new Map(discovered.map((repository) => [repository.localPath, repository]));
    return records.flatMap((record) => {
      const repository = discoveredByPath.get(record.localPath);
      return repository ? [{ record, discovered: repository }] : [];
    });
  }
  ...
  async function resolveTaskPushRepositoryCapability(
    ...
    discovered?: Pick<DiscoveredGitRepository, 'context' | 'clean'>,
  ) {
    let repository = discovered?.context ?? (await getGitRepositoryContext(registeredRepository.localPath));
    let clean = discovered?.clean ?? (await getGitWorktreeClean(registeredRepository.localPath, projectRepositoryIgnoredPaths(project.id, registeredRepository.id, registeredRepository.localPath)));
  ...
  function scheduleTaskIntegrationAiFinalization(runId: string, conversationId: string): void {
    const operation = taskConflictAiOperations.get(conversationId);
    if (!operation || operation.runId !== runId || operation.finalizing) return;
    operation.finalizing = true;
    taskIntegrationAiRuns.update(runId, { state: 'finalizing' });
    const previous = taskConflictAiFinalizationQueues.get(operation.integrationId) ?? Promise.resolve();
    const current = previous.catch(() => undefined).then(() => finalizeTaskIntegrationAfterAi(runId, conversationId));
    taskConflictAiFinalizationQueues.set(operation.integrationId, current);
    void current.finally(() => {
      taskConflictAiOperations.delete(conversationId);
      if (taskConflictAiFinalizationQueues.get(operation.integrationId) === current) taskConflictAiFinalizationQueues.delete(operation.integrationId);
    });
  }

  async function finalizeTaskIntegrationAfterAi(runId: string, conversationId: string): Promise<void> {
    try {
      const aiRun = taskIntegrationAiRuns.getById(runId);
      if (!aiRun?.integrationPath) throw nativeApiError('ZEUS_TASK_INTEGRATION_PATH_UNAVAILABLE', 'AI 冲突处理隔离现场不可用。');
      const currentIntegration = taskIntegrations.getById(aiRun.integrationId);
      if (!currentIntegration) {
        taskIntegrationAiRuns.update(runId, { state: 'superseded', lastError: '本次本地合入已由另一条处理会话完成。' });
        await db.save();
        return;
      }
      ...
      if (integration.state !== 'conflicted') {
        ...
        await cleanupPreparedTaskWorktree({
          repositoryPath: workspace.repositoryPath || project.localPath,
          worktreePath: aiRun.integrationPath,
          branchName: workspace.branchName,
          removeBranch: false,
        });
        await db.save();
      ...
      await assertTaskIntegrationStillCurrent(project, workspace, integration);
      const commit = await completeTaskIntegrationCommit({
        integrationPath: aiRun.integrationPath,
        mode: integration.mode,
        commitMessage: `${task.taskCode}: 合入 ${workspace.branchName}`,
      });
      const finalized = await finalizeTaskBranchIntegration({
      ...
      if (integration.integrationPath && resolve(integration.integrationPath) !== resolve(aiRun.integrationPath)) {
        await cleanupPreparedTaskWorktree({
          repositoryPath: workspace.repositoryPath || project.localPath,
      ...
      const pendingLocalSync = finalized.localSyncStatus === 'pending';
      taskIntegrations.update(integration.id, {
        integrationPath: pendingLocalSync ? aiRun.integrationPath : null,
        resultHeadSha: finalized.resultHeadSha,
        state: pendingLocalSync ? 'pending_local_sync' : 'merged',
        localSyncStatus: finalized.localSyncStatus,
        localHeadSha: finalized.localHeadSha,
      ...
      taskIntegrationAiRuns.update(runId, { state: 'completed', resultHeadSha: finalized.resultHeadSha, lastError: null });
      if (pendingLocalSync) {
        recordTaskEvent({
          taskId: task.id,
          eventType: 'task.git_integration.local_sync_pending',
      ...
      const aiRun = taskIntegrationAiRuns.getById(runId);
      if (aiRun) taskIntegrationAiRuns.update(runId, { state: 'failed', lastError: error instanceof Error ? error.message : 'AI 本地合入未完成。' });
      const integration = aiRun ? taskIntegrations.getById(aiRun.integrationId) : undefined;
      if (integration && integration.state !== 'merged' && integration.state !== 'pending_local_sync') {
        taskIntegrations.update(integration.id, {
