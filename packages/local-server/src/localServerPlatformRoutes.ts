...
export async function registerLocalServerPlatformRoutes(dependencies: LocalServerPlatformRouteDependencies): Promise<{
  ...
  registerGraphConversationCommandRoutes({
    ...
    operations: {
      ...
      startProjectConversation: ({ prepared, value, operationIdentity }) => {
        const { project } = requirePreparedGraphProject(prepared);
        return executeProjectConversationIdempotent(project, value, operationIdentity);
      },
      ...
      startTaskConversation: ({ prepared, value, operationIdentity }) => {
        const { project, task } = requirePreparedGraphTask(prepared);
        return executeTaskConversationIdempotent(project, task, value, operationIdentity);
      },
