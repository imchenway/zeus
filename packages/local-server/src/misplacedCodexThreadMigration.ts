...
async function copyOrAdoptThreadFile(input: { sourcePath: string; sourceSnapshot: FileSnapshot; targetPath: string; providerThreadId: string }): Promise<{ result: 'copied' | 'existing'; sha256: string }> {
...
async function verifyExistingTarget(input: { sourcePath: string; sourceSnapshot: FileSnapshot; targetPath: string; providerThreadId: string }): Promise<{ result: 'existing'; sha256: string }> {
