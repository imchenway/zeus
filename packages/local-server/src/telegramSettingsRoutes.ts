...
import { createTelegramBotMessageClient, dispatchTelegramUpdate, getTelegramConfigurationState, type TelegramMessageSender, type TelegramPollingService, type TelegramUpdate } from './telegramAdapter.js';
import { telegramChildOperation, telegramCommandTypes, type TelegramCommandApplication, type TelegramCommandRequest } from './telegramCommandApplication.js';
import type {
...
export function registerTelegramSettingsRoutes(options: {
  ...
  appendAuditLog: (entry: { actorType: string; action: string; resourceType: string; resourceId: string; payload: Record<string, unknown> }) => void;
  ...
  server.put(
    '/api/telegram/notification-settings',
    async (
  ...
  server.post(
    ...
    async (
      ...
      reply,
    ): Promise<TelegramTestConnectionResult | unknown> => {
      try {
  ...
  server.put(
    '/api/telegram/security-settings',
    async (
      request: FastifyRequest<{
  ...
  server.patch(
    ...
    async (
      ...
      reply,
    ): Promise<TelegramSettingsSnapshot | unknown> => {
      try {
