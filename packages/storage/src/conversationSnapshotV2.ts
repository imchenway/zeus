...
import {
    ...
    type ConversationSessionMetricsSnapshot,
    readConversationSessionMetrics
} from './conversationExecutionStore.js';
...
export class ConversationSnapshotV2Repository {
  ...
  readSessionMetrics(conversationIdValue: string): ConversationSessionMetricsSnapshot {
    const conversationId = requiredIdentity(conversationIdValue, 'conversationId');
      const conversation = this.db.get<{ provider_state: string }>(`SELECT provider_state
                                                                    FROM conversations
