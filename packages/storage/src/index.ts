  ...
  replaceCompletedPiAgentMessage(input: { providerThreadId: string; providerItemId: string; textContent: string; updatedAt: string }): ZeusConversationItemRecord | undefined {
    this.db.execute(
      `UPDATE conversation_items
       SET text_content = ?, phase = 'final_answer', updated_at = ?
       WHERE provider_thread_id = ? AND provider_item_id = ?
         AND item_type = 'agentMessage' AND status = 'completed' AND agent_kind = 'pi'`,
