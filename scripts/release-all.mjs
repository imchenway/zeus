...
/** 按稳定版本号选择发布基线，避免发布时间变化导致回退。 */
function readLatestStableRelease() {
  const pages = JSON.parse(gh(['api', `repos/${repository}/releases?per_page=100`, '--paginate', '--slurp']));
  const releases = pages.flat().filter((release) => !release.draft && !release.prerelease && /^v\d+\.\d+\.\d+$/u.test(release.tag_name ?? ''));
  ...
  if (!value) throw new Error('尚无 Zeus 稳定发行版，无法确认本次发布基线。');
  return { tag: value.tag_name, version: value.tag_name.slice(1), publishedAt: value.published_at, url: value.html_url };
}
