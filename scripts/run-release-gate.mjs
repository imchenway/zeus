...
async function main() {
  ...
  const dmgPath = join(repositoryRoot, 'dist', `Zeus-${expectedVersion}-${architecture}.dmg`);
  const manifestPath = join(repositoryRoot, 'dist', 'zeus-release-manifest.json');
  const generatedCaskPath = join(repositoryRoot, 'dist', 'homebrew', 'zeus.rb');
  const appPath = join(repositoryRoot, 'dist', architecture === 'arm64' ? 'mac-arm64' : 'mac', 'Zeus.app');
