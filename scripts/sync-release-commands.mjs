...
const expectedCommandNames = ['zeus-release-notes', 'zeus-release-prepare', 'zeus-release-gate', 'zeus-release-publish'];
...
async function main() {
  ...
  if (validateOnly) {
    ...
    console.log(`四条发布命令定义验证通过：${reportPath}`);
  ...
  if (!applyCommands) {
    ...
    console.log(`待创建 ${plan.creates.length} 条、更新 ${plan.updates.length} 条、保持 ${plan.unchanged.length} 条。`);
    console.log(`ZEUS_ARTIFACT_FILE=${planPath}`);
    return;
  }
  ...
  if (verification.blockers.length > 0 || verification.creates.length > 0 || verification.updates.length > 0) {
  ...
  writeFileSync(
    ...
    buildPlanReport({
      ...
      applyCommands,
      plan,
      result: `同步并回读通过：创建 ${plan.creates.length} 条，更新 ${plan.updates.length} 条，保持 ${plan.unchanged.length} 条`,
    }),
    { mode: 0o600 },
  );
  console.log(`四条发布命令已同步并回读验证：${planPath}`);
  console.log(`ZEUS_ARTIFACT_FILE=${planPath}`);
}
...
function buildPlanReport(input) {
  return [
    ...
    '## 安全边界',
    '',
    '- 只处理声明文件中的四条项目命令，不创建全局命令。',
    '- 不删除未声明命令，不修改项目 Shell 或 Git 写入权限。',
