...
try {
  ...
  const restartedProvider = createRestartProbeManager({
    providerThreadId,
    turnIds: [secondProviderTurnId, quotaRecoveryTurnId],
...
function createRestartProbeManager(input: { providerThreadId: string; turnIds: string[]; initialTurns: CodexTurnSnapshot[] }): {
  ...
  return {
    ...
    async failLatestTurn() {
      ...
      await Promise.all([...listeners].map((listener) => listener({ generationId, sequence: 1, method: 'turn/completed', params: { threadId: input.providerThreadId, turn }, receivedAt: turn.completedAt! })));
    },
    get readThreadCalls() {
