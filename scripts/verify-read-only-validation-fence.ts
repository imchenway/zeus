...
import { createZeusDataLayout, inspectReadOnlyValidationManifest, startZeusLocalServer, verifyReadOnlyValidationDescriptor, type BrowserAutomationPort } from '../packages/local-server/src/index.ts';
...
try {
  ...
  assertProbe(
    diagnostics.statusCode === 200 &&
      ...
      diagnostics.body.skipped.length === 14 &&
      new Set(diagnostics.body.skipped.map((entry) => (isRecord(entry) ? entry.id : null))).size === 14,
    '诊断接口必须给出同一副本身份和 14 项互异的启动副作用跳过证据。',
  );
  assertProbe(
